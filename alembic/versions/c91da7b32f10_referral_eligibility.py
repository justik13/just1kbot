...
revision = "c91da7b32f10"
down_revision = "b62fe1349c20"
...
def downgrade():
    count = (
        ...
        .execute(
            sa.text(
                "SELECT count(*) FROM referral_eligibilities WHERE status='blocked'"
            )
        )
        .scalar_one()
    )
    if count:
        raise RuntimeError(
            "Cannot downgrade while blocked referral eligibility requires review"
        )
