...
revision = "b62fe1349c20"
down_revision = "ab17c4e92901"
branch_labels = depends_on = None


def upgrade():
    op.drop_constraint(
        "referral_rewards_referred_user_id_key", "referral_rewards", type_="unique"
    )
    op.add_column(
        "referral_rewards",
    ...
    op.create_index(
        "uq_referral_rewards_first_user",
        "referral_rewards",
        ["referred_user_id"],
        unique=True,
        postgresql_where=sa.text("is_first = true"),
    )
...
def downgrade():
    count = (
        op.get_bind()
        .execute(sa.text("SELECT count(*) FROM referral_rewards WHERE is_first=false"))
        .scalar_one()
    )
    if count:
        raise RuntimeError("Cannot downgrade: recurring referral rewards exist")
    op.execute(
        "DELETE FROM entitlement_entries WHERE metadata->>'legacy_backfill'='true'"
    )
    op.drop_index("uq_referral_rewards_first_user", table_name="referral_rewards")
    op.drop_column("referral_rewards", "is_first")
    op.create_unique_constraint(
        "referral_rewards_referred_user_id_key",
        "referral_rewards",
        ["referred_user_id"],
    )
