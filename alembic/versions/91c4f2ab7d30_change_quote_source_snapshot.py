...
def upgrade():
    ...
    op.create_check_constraint("ck_tariff_quotes_lifecycle_timestamps", "tariff_quotes", "(status = 'consumed' AND consumed_at IS NOT NULL AND manual_review_at IS NULL) OR (status = 'manual_review' AND manual_review_at IS NOT NULL) OR (status IN ('active','expired','cancelled') AND consumed_at IS NULL AND manual_review_at IS NULL)")
    ...
    op.execute("""CREATE FUNCTION reject_quote_economic_change() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN
      ...
      IF NEW.status <> OLD.status AND NOT ((OLD.status='active' AND NEW.status IN ('expired','cancelled','consumed','manual_review')) OR (OLD.status='expired' AND NEW.status IN ('consumed','manual_review')) OR (OLD.status='cancelled' AND NEW.status='manual_review') OR (OLD.status='consumed' AND NEW.status='manual_review')) THEN RAISE EXCEPTION 'invalid quote lifecycle transition'; END IF;
      RETURN NEW; END $$""")
