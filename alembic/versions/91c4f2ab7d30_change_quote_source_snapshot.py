...
def upgrade():
    ...
    op.create_check_constraint("ck_tariff_quotes_change_source_snapshot", "tariff_quotes", "operation_type <> 'change' OR (balance_as_of IS NOT NULL AND source_subscription_end IS NOT NULL AND source_balance_fingerprint IS NOT NULL AND source_entitlement_entry_ids IS NOT NULL AND source_ledger_entry_ids IS NOT NULL)")
    ...
    op.create_check_constraint("ck_tariff_quotes_source_arrays", "tariff_quotes", "(source_entitlement_entry_ids IS NULL OR jsonb_typeof(source_entitlement_entry_ids)='array') AND (source_ledger_entry_ids IS NULL OR jsonb_typeof(source_ledger_entry_ids)='array')")
