"""Update entitlement_entries check constraints for referral types
...
def downgrade() -> None:
    ...
    op.create_check_constraint(
        'ck_entitlement_entries_type',
        'entitlement_entries',
        "entry_type IN ('account_purchase_grant', 'manual_grant', 'tariff_change')",
    )
    op.create_check_constraint(
        'ck_entitlement_entries_shape',
        'entitlement_entries',
        "(((entry_type IN ('account_purchase_grant', 'manual_grant')) AND (days_delta > 0) AND (reversed_entry_id IS NULL) AND ((hours_delta IS NULL) OR (hours_delta = (days_delta * 24)))) OR ((entry_type = 'tariff_change') AND (source_type = 'quote') AND (days_delta = 0) AND (hours_delta > 0) AND (reversed_entry_id IS NULL)))",
    )
