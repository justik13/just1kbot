...
• 🔑 <b>Ключ</b> — основной и самый простой способ.
• 📁 <b>Скачать файлом</b> — резервный способ (если ключ не подходит).""",

    "DEVICE_SHOW_KEY": """🔑 <b>Ключ подключения для {device_name}:</b>

<code>{raw_config}</code>

<i>💡 Нажмите на моноширинный текст выше, чтобы скопировать ключ.
Если этот ключ не подходит в вашем приложении, используйте кнопку «📥 Скачать файлом».</i>""",
...
• Откройте готовое устройство и выберите один из способов:
  — «📥 Скачать файлом» — получить файлы конфигурации (.vpn и .conf).
  — «🔑 Показать ключ» — скопировать ключ vpn:// для вставки в приложение.

📌 <b>2. Какие приложения использовать для подключения?</b>
• <b>AmneziaVPN</b> — основное приложение. Файл .vpn или ключ vpn://. Windows, macOS, Linux, Android, iOS.
• <b>AmneziaWG</b> — лёгкий клиент. Только файл .conf. Android, iOS, Windows.
