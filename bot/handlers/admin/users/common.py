...
async def _get_active_tariffs(session: AsyncSession) -> list[Tariff]:
    result = await session.execute(select(Tariff).where(Tariff.is_active.is_(True)).order_by(Tariff.device_limit))
...
async def _get_tariff_groups(session: AsyncSession) -> dict[int, list[Tariff]]:
    ...
    for tariff in tariffs:
        groups.setdefault(tariff.device_limit, []).append(tariff)
    return groups
...
async def _get_user_with_profiles(session: AsyncSession, telegram_id: int):
    stmt = select(User).where(User.telegram_id == telegram_id).options(selectinload(User.profiles))
...
async def _build_users_list_text_and_kb(
    ...
    rendered = f"{header}👥 <b>Управление пользователями</b> (Стр. {page}/{total_pages}, всего: {total})\n\n"
    ...
    for f_code, f_name, f_param in filters:
        ...
        builder.button(text=label, callback_data=f"admin_users_filter:{f_code}:{f_param}:1")
    ...
    if not users:
        ...
        for user in users:
            ...
            button_text = truncate_button_text(f"{status}{ban} {username} | {days} | {profiles_count} устр.")
            builder.button(text=button_text, callback_data=f"admin_user_card:{user.telegram_id}")

    nav_buttons = 0
    if page > 1:
        builder.button(text="◀️ Назад", callback_data=f"admin_users_filter:{filter_type}:{filter_param}:{page - 1}")
        nav_buttons += 1
    if page < total_pages:
        builder.button(text="Вперед ▶️", callback_data=f"admin_users_filter:{filter_type}:{filter_param}:{page + 1}")
        nav_buttons += 1

    builder.button(text="🔍 Поиск по @username / ID", callback_data="admin_users_search")
...
async def _render_user_card(callback: CallbackQuery, user: User, session: AsyncSession):
    ...
    current_time = now_utc()
    rendered = format_user_card_text(user, profiles, referrals, current_time, real_balance=int(balance.real_available), bonus_balance=int(balance.bonus_available))
    try:
        await callback.message.edit_text(rendered, reply_markup=get_admin_user_card_keyboard(user.telegram_id, user.is_banned), parse_mode="HTML")
    except TelegramBadRequest as e:
        logger.debug(f"_render_user_card edit_text failed: {e}")


async def _show_user_card_edit(message, user, session: AsyncSession, notice: str | None = None):
    from database.repositories.account_ledger_repo import get_account_balance
    profiles = await get_user_profiles(session, user.id)
    referrals = await get_user_referrals(session, user.telegram_id)
    balance = await get_account_balance(session, user_id=user.id)
    ...
    await render_hub(message.bot, message.chat.id, rendered, get_admin_user_card_keyboard(user.telegram_id, user.is_banned), trigger_message_id=trigger_message_id)
