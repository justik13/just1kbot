...
stage=initializing result="" cutover_phase=before_stop advisory_pid="" advisory_fd="" advisory_backend_pid="" cleanup_running=false operation_created=false rename_count=0
...
safe_production_db(){ valid_db "$1" && [[ $1 != rehearsal_* && $1 != candidate_* && $1 != previous_* && $1 != failed_* && $1 != just1kbot_rehearsal_* && $1 != just1kbot_candidate_* && $1 != just1kbot_previous_* && $1 != just1kbot_failed_* ]]; }
...
db_exists(){
 valid_db "$1" || return 1; local count
 count=$(psql_m -At -v "target_db=$1" <<'SQL'
SELECT count(*) FROM pg_database WHERE datname = :'target_db';
SQL
 ) || return 1; [[ $count == 1 ]]
}
database_size(){
 valid_db "$1" || return 1; local value
 value=$(psql_m -At -v "target_db=$1" <<'SQL'
SELECT pg_database_size(:'target_db');
SQL
 ) || return 1; [[ $value =~ ^[1-9][0-9]*$ ]] || return 1; printf '%s\n' "$value"
}
database_owner(){
 valid_db "$1" || return 1; local value
 value=$(psql_m -At -v "target_db=$1" <<'SQL'
...
SQL
 ) || return 1; [[ $value =~ ^[A-Za-z_][A-Za-z0-9_$-]*$ ]] || return 1; printf '%s\n' "$value"
}
database_attributes(){
 valid_db "$1" || return 1; local value
 value=$(psql_m -At -F $'\t' -v "target_db=$1" <<'SQL'
...
rename_db(){
 valid_db "$1" && valid_db "$2" && advisory_lock_held || return 1; rename_count=$((rename_count+1))
...
validate_manifest(){
 ...
 MANIFEST="$path" EXPECTED_PRODUCTION_DB="$PRODUCTION_DB" python3 - <<'PY'
import json,os,pathlib,re
p=pathlib.Path(os.environ['MANIFEST']); d=json.loads(p.read_text())
required={'format_version','operation_id','started_utc','finished_utc','code_git_sha','source_artifact_basename','source_artifact_sha256','source_manifest_revision','candidate_database','candidate_database_state','original_production_database','previous_database_quarantine_name','failed_candidate_name','emergency_backup_basename','emergency_backup_path','emergency_backup_sha256','emergency_backup_pin','finalize_backup_basename','finalize_backup_path','finalize_backup_sha256','stage','result','rollback_attempted','rollback_result','service_health_result','advisory_lock_cleanup'}
...
production=d['original_production_database']
if production!=os.environ['EXPECTED_PRODUCTION_DB'] or not re.fullmatch(r'[a-z][a-z0-9_]{0,62}',production): raise SystemExit(1)
if re.match(r'(?:just1kbot_)?(?:candidate|rehearsal|previous|failed)(?:_|$)',production): raise SystemExit(1)
...
if d['failed_candidate_name'] and (not re.fullmatch(r'just1kbot_failed_restore_[a-z0-9_]+',d['failed_candidate_name']) or not d['failed_candidate_name'].startswith('just1kbot_failed_restore_'+stamp+'_')): raise SystemExit(1)
if d['candidate_database_state'] not in {'planned','present','removed','production','failed_quarantine'}: raise SystemExit(1)
terminal_stages={'failed_safe':{'recover_original_service','failed_safe_pre_stop'},'success':{'success'},'rolled_back':{'automatic_rollback','manual_rollback'},'finalized':{'finalized'}}
if d['result'] in terminal_stages and d['stage'] not in terminal_stages[d['result']]: raise SystemExit(1)
PY
}
...
start_advisory_lock(){
 ...
 local deadline=$((SECONDS+30))
  while ((SECONDS<deadline)); do
   if [[ $(cat "$workspace/advisory.ready" 2>/dev/null) =~ ^advisory_lock=acquired\ backend_pid=([0-9]+)$ ]]; then advisory_backend_pid=${BASH_REMATCH[1]}; return 0; fi
   kill -0 "$advisory_pid" 2>/dev/null || return 1; sleep .1
  done
 return 1
}
advisory_lock_held(){
 [[ -n $advisory_pid && -n $advisory_backend_pid ]] && kill -0 "$advisory_pid" 2>/dev/null || return 1
 local held; held=$(psql_m -At -v "lock_pid=$advisory_backend_pid" <<'SQL'
SELECT count(*) FROM pg_locks
WHERE pid = :'lock_pid'::integer AND locktype = 'advisory' AND granted;
SQL
 ) || return 1; [[ $held == 1 ]]
}
stop_advisory_lock(){
 [[ -n $advisory_pid ]] || return 0
 local status=released deadline healthy=true; advisory_lock_held || { status=helper_died; healthy=false; }
 eval "exec ${advisory_fd}>&-" 2>/dev/null || true; deadline=$((SECONDS+${RESTORE_ADVISORY_STOP_SECONDS:-5}))
 while kill -0 "$advisory_pid" 2>/dev/null && ((SECONDS<deadline)); do sleep .1; done
 if kill -0 "$advisory_pid" 2>/dev/null; then status=terminated; kill -TERM "$advisory_pid" 2>/dev/null || true; deadline=$((SECONDS+2)); while kill -0 "$advisory_pid" 2>/dev/null && ((SECONDS<deadline)); do sleep .1; done; fi
 if kill -0 "$advisory_pid" 2>/dev/null; then status=killed; kill -KILL "$advisory_pid" 2>/dev/null || true; fi
 wait "$advisory_pid" 2>/dev/null || { status=helper_failed; healthy=false; }; advisory_pid=""; advisory_fd=""; advisory_backend_pid=""; manifest_update advisory_lock_cleanup="$status" || true; $healthy
}
...
recover_original_service_before_swap(){
 validate_manifest "$manifest_file" || { safe_diagnostics; return "$CRITICAL_RECOVERY_EXIT"; }
...
cleanup(){
 ...
 if ((rc!=0)) && $operation_created && [[ -n $manifest_file && -z $result && $cutover_phase == before_stop ]]; then
  if verify_safe_pre_swap_state; then result=failed_safe; manifest_update stage=failed_safe_pre_stop result=failed_safe service_health_result=success finished_utc="$(date -u +%FT%TZ)"; else result=requires_manual_recovery; manifest_update result="$result" service_health_result=failure finished_utc="$(date -u +%FT%TZ)"; rc=$CRITICAL_RECOVERY_EXIT; fi
 fi
 stop_advisory_lock
 if ((rc!=0)) && [[ $result == failed_safe && -n $candidate_db && $candidate_db == just1kbot_candidate_* ]]; then
  if [[ -z $emergency_artifact ]]; then dropdb --force --if-exists --maintenance-db="$MAINTENANCE_DB" "$candidate_db" >/dev/null 2>&1 || true; if db_exists "$candidate_db"; then manifest_update candidate_database_state=present; else manifest_update candidate_database_state=removed; fi
  else manifest_update candidate_database_state=present; fi
 fi
...
check_incomplete(){
 local f status c p x cstate; shopt -s nullglob
 for f in "$OPERATION_DIR"/*.json; do
  validate_manifest "$f" || { fail incomplete_or_invalid_operation 12; return; }
  mapfile -d '' -t x < <(MANIFEST="$f" python3 -c 'import json,os;d=json.load(open(os.environ["MANIFEST"]));[print(d[k],end="\0") for k in ("result","candidate_database","previous_database_quarantine_name","failed_candidate_name","candidate_database_state")]')
  status=${x[0]}; c=${x[1]}; p=${x[2]}; cstate=${x[4]}
  [[ $status == failed_safe || $status == success || $status == rolled_back || $status == finalized ]] || { fail incomplete_operation_exists 12; return; }
  case $status in
   failed_safe) db_exists "$PRODUCTION_DB" && ! db_exists "$p" && service is-active >/dev/null 2>&1 && { [[ $cstate == present ]] && db_exists "$c" || [[ $cstate == removed ]] && ! db_exists "$c"; } || { fail operation_database_state_mismatch 12; return; };;
...
inspect_incomplete(){
 local f invalid=false; shopt -s nullglob
 for f in "$OPERATION_DIR"/*.json; do
  if ! validate_manifest "$f"; then printf 'operation=%s result=invalid stage=manifest_production_mismatch recommended_action=repair_manifest_offline\n' "$(basename "$f" .json)"; invalid=true; continue; fi
...
rollback_database_names(){
 ...
 if service start >/dev/null && service is-active >/dev/null 2>&1 && health; then cutover_phase=resolved; stop_advisory_lock || return 1; return 0; fi
 return 1
}
manual_rollback(){
 ...
 previous_db=${x[1]}; operation_stamp=${operation_id:8:15}; operation_stamp=${operation_stamp//T/}; failed_db=${x[2]:-just1kbot_failed_restore_${operation_stamp}_$(printf %04x "$RANDOM")}; valid_db "$failed_db" && [[ $failed_db == just1kbot_failed_restore_* ]] || return 1
...
load_db_env || exit 1
case $mode in
 inspect)inspect_incomplete;rc=$?;result=success;exit "$rc";;
 rollback)workspace=$(mktemp -d "${TMPDIR:-/tmp}/just1kbot-rollback.XXXXXX");manual_rollback "$rollback_id" "$rollback_confirm";exit $?;;
 finalize)finalize "$finalize_id" "$finalize_confirm";exit $?;;
 test_helpers)
  [[ ${RESTORE_TEST_MODE:-false} == true ]] || exit 2; helper_db=${RESTORE_TEST_HELPER_DATABASE:-}; valid_db "$helper_db" || exit 2
  db_exists "$helper_db"; printf 'exists=true\n'; database_size "$helper_db" | sed 's/.*/size=positive/'; printf 'owner=%s\n' "$(database_owner "$helper_db")"; database_attributes "$helper_db" | sed 's/.*/attributes=present/'; terminate_db "$helper_db"; printf 'terminate=success\n'; result=success; exit 0;;
esac
...
fingerprint(){ stat -Lc '%d:%i:%s' "$1";printf ':%s' "$(sha256sum "$1"|awk '{print $1}')";};artifact_fp=$(fingerprint "$canonical");sidecar_fp=$(fingerprint "$sidecar");source_sha=$(sha256sum "$canonical"|awk '{print $1}');size=$(stat -c %s "$canonical");dbsize=$(database_size "$PRODUCTION_DB");required=$((size*6+dbsize*3+1073741824))
...
manifest_update format_version=1 operation_id="$operation_id" started_utc="$(date -u +%FT%TZ)" finished_utc= code_git_sha="$(git -C "$PROJECT_DIR" rev-parse HEAD 2>/dev/null||echo unavailable)" source_artifact_basename="$(basename "$canonical")" source_artifact_sha256="$source_sha" source_manifest_revision= candidate_database="$candidate_db" candidate_database_state=planned original_production_database="$PRODUCTION_DB" previous_database_quarantine_name="$previous_db" failed_candidate_name= emergency_backup_basename= emergency_backup_path= emergency_backup_sha256= emergency_backup_pin= finalize_backup_basename= finalize_backup_path= finalize_backup_sha256= stage=verification result=in_progress rollback_attempted=false rollback_result=not_attempted service_health_result=not_checked advisory_lock_cleanup=not_started
...
owner=$(database_owner "$PRODUCTION_DB");IFS=$'\t' read -r encoding collate ctype<<<"$(database_attributes "$PRODUCTION_DB")";createdb --maintenance-db="$MAINTENANCE_DB" --owner="$owner" --template=template0 --encoding="$encoding" --lc-collate="$collate" --lc-ctype="$ctype" "$candidate_db";manifest_update candidate_database_state=present;pg_restore --exit-on-error --no-owner --no-acl --dbname="$candidate_db" "$workspace/verified/dump.custom">/dev/null
...
create_pinned_backup "$operation_id" "$workspace/emergency.result"||exit 1;emergency_artifact=${backup_result[0]};emergency_sha=${backup_result[1]}
manifest_update emergency_backup_basename="$(basename "$emergency_artifact")" emergency_backup_path="$emergency_artifact" emergency_backup_sha256="$emergency_sha" emergency_backup_pin="$operation_id" stage=emergency_created
"$VERIFY_BACKUP" "$emergency_artifact">/dev/null||exit 1;manifest_update stage=emergency_verified
"$REHEARSAL_COMMAND" "$emergency_artifact">/dev/null||exit 1;manifest_update stage=emergency_rehearsed
manifest_update stage=swap
...
cutover_phase=swapped;stage=post_swap_health;manifest_update stage="$stage" candidate_database_state=production;service start>/dev/null||true
if health;then cutover_phase=resolved;if ! stop_advisory_lock;then result=requires_manual_recovery;manifest_update result="$result" service_health_result=failure;exit "$CRITICAL_RECOVERY_EXIT";fi;result=success;manifest_update stage=success result=success rollback_attempted=false rollback_result=not_attempted service_health_result=success finished_utc="$(date -u +%FT%TZ)";exit 0;fi
...
if ! stop_service;then result=rollback_failed;manifest_update result=rollback_failed rollback_result=failure service_health_result=failure;safe_diagnostics;exit "$CRITICAL_ROLLBACK_EXIT";fi
terminate_db "$PRODUCTION_DB"||{ result=rollback_failed;manifest_update result="$result" rollback_result=failure;exit "$CRITICAL_ROLLBACK_EXIT";};rename_db "$PRODUCTION_DB" "$failed_db"||{ result=rollback_failed;manifest_update result="$result" rollback_result=failure;exit "$CRITICAL_ROLLBACK_EXIT";};manifest_update candidate_database_state=failed_quarantine;rename_db "$previous_db" "$PRODUCTION_DB"||{ result=rollback_failed;manifest_update result="$result" rollback_result=failure;exit "$CRITICAL_ROLLBACK_EXIT";}
if service start>/dev/null&&health;then cutover_phase=resolved;if ! stop_advisory_lock;then result=rollback_failed;manifest_update result="$result" rollback_result=failure service_health_result=failure;exit "$CRITICAL_ROLLBACK_EXIT";fi;result=rolled_back;manifest_update result=rolled_back rollback_result=success service_health_result=success finished_utc="$(date -u +%FT%TZ)";exit 20;fi
