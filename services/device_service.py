...
class DeviceService:
    ...
    async def create_device(session: AsyncSession, user: User, server_id: int,
        ...
        if server_count >= server.max_clients:
    ...
    async def delete_device(session: AsyncSession, profile: VPNProfile,
        ...
        existing = (await session.execute(select(APIOperation).where(
            APIOperation.idempotency_key == f"delete-peer:{profile.id}:{profile.peer_id}"))).scalar_one_or_none()
        if existing:
            return True
        ...
        await enqueue_api_operation(session, operation_type="delete_peer",
            ...
            payload={"managed_workflow": True, "reason": "device_delete"})
        return True
