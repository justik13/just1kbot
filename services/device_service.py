...
def _cleanup_locks(now: float) -> None:
    ...
    if old_users or old_servers:
        logger.debug(
            ...
            len(old_users), len(old_servers),
            len(_user_locks), len(_server_locks),
        )
...
async def _queue_failed_rollback_deletion(
    server, peer_id: str, user_id: int, reason: str,
) -> None:
...
class DeviceService:
        ...
        try:
            try:
                ...
                logger.warning(
                    ...
                    "falling back to local lock: %s", e,
                )
            ...
            async with lock:
                ...
                clean_device_name = (
                    re.sub(r"[^a-zA-Z0-9]", "", device_name)[:10]
                    or "Device"
                )
                client_name = (
                    ...
                    try:
                        ...
                        if not deleted:
                            await _queue_failed_rollback_deletion(
                                server, peer_id, user.telegram_id,
                                "invalid_config_delete_false",
                            )
                    except Exception as rollback_error:
                        await _queue_failed_rollback_deletion(
                            server, peer_id, user.telegram_id,
                            f"invalid_config_delete_error: "
                            f"{rollback_error}",
                        )
                    raise InvalidConfig(
                ...
                try:
                    ...
                    try:
                        ...
                        await _queue_failed_rollback_deletion(
                            server, peer_id, user.telegram_id,
                            f"integrity_error_delete_error: "
                            f"{rollback_error}",
                        )
                    raise
                    ...
                    if peer_id and client:
                        try:
                            ...
                            await _queue_failed_rollback_deletion(
                                server, peer_id, user.telegram_id,
                                f"unexpected_error_delete_error: "
                                f"{rollback_error}",
                            )
    ...
    # ──────────────────────────────────────────────────────────
    # ИСПРАВЛЕНО: сначала DB delete (savepoint), потом API.
    # Если API failed → PendingAPIDeletion.
    # Если DB failed → ничего не трогаем на API.
    # ──────────────────────────────────────────────────────────
    @staticmethod
    async def delete_device(
        ...
        # 1. Сначала удаляем профиль из БД (в savepoint)
        try:
            async with session.begin_nested():
                ...
                try:
                    await AuditService.log_action(
                        ...
                        details=(
                            ...
                            f"server="
                            f"{server.name if server else 'unknown'}"
                        ),
                    )
                except Exception:
                    pass
            ...
            logger.exception(
                "Failed to delete profile %s from DB",
                profile.id,
            )
            return False

        # 2. Потом удаляем на API
        if server and server.api_url and server.api_key:
            ...
            if not deleted:
                # API не удалил → ставим в pending queue
                pending = PendingAPIDeletion(
                    server_name=server.name,
                    api_url=server.api_url,
                    api_key=server.api_key,
            ...
            # Сервер уже удалён из БД — нечего удалять на API
            pass
