...
def _cleanup_locks(now: float) -> None:
    ...
    if old_users or old_servers:
        logger.debug(
            "Device locks cleanup: removed %s user locks, %s server locks. "
            "Remaining: %s user, %s server",
            len(old_users),
            len(old_servers),
            len(_user_locks),
            len(_server_locks),
...
class DeviceService:
    ...
    async def create_device(
        ...
        try:
            ...
            async with lock:
                ...
                clean_device_name = (
                    re.sub(r"[^a-zA-Z0-9]", "", device_name)[:10] or "Device"
                )

                client_name = (
                    f"tg_{user.telegram_id}_{clean_device_name}_{short_hash}"
                )
    ...
    async def delete_device(
        ...
        try:
            ...
            logger.exception(
                "Failed to delete profile %s, pending deletion kept",
                profile.id,
            )
