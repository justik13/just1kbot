...
def parse_provider_captured_at(value) -> datetime:
    ...
    if candidate.endswith("Z"): candidate=candidate[:-1]+"+00:00"
    try: parsed=datetime.fromisoformat(candidate)
    except ValueError as exc: raise ValueError("captured_at_invalid") from exc
...
async def apply_provider_transition(session,payment,data,*,source,event_type=None):
    ...
    if observed=="succeeded":
        ...
        if payment.tariff_quote_id:
            try: captured_at=parse_provider_captured_at(data.get("captured_at"))
            except ValueError as exc:
                payment.provider_status="succeeded"; payment.reconciliation_status="manual_review"; payment.fulfillment_status="manual_review"; payment.fulfillment_last_error_code=str(exc)
                quote=await session.scalar(select(TariffQuote).where(TariffQuote.id==payment.tariff_quote_id).with_for_update())
                if quote:
                    quote.status="manual_review"; quote.manual_review_at=quote.manual_review_at or now_utc(); quote.diagnostic_reason=str(exc)
                session.add(PaymentEvent(payment_id=payment.id,event_type="provider_captured_at_invalid",provider_status=observed,reason=str(exc),source=source))
            ...
            if payment.provider_confirmed_at and payment.provider_confirmed_at != captured_at:
                payment.provider_status="succeeded"; payment.reconciliation_status="manual_review"; payment.fulfillment_status="manual_review"; payment.fulfillment_last_error_code="captured_at_changed"
                quote=await session.scalar(select(TariffQuote).where(TariffQuote.id==payment.tariff_quote_id).with_for_update())
                if quote: quote.status="manual_review"; quote.manual_review_at=quote.manual_review_at or now_utc(); quote.diagnostic_reason="captured_at_changed"
                session.add(PaymentEvent(payment_id=payment.id,event_type="provider_captured_at_conflict",provider_status=observed,reason="captured_at_changed",source=source))
                return ProviderTransition("conflict",observed,reason="captured_at_changed")
