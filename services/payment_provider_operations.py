...
def provider_transition_source(claim):
    ...
    if claim.operation_type == "create_payment":
        return (
            "provider_get_payment"
            if claim.external_id
            else "provider_create_payment_post"
        )
...
async def ensure_operation(session, payment, operation_type):
    if operation_type != "cancel_payment":
        raise ValueError(operation_type)
    key = f"payment-cancel:{payment.id}:v1"
    payload = {"provider_payment_id": payment.external_id}
    stmt = (
        insert(PaymentProviderOperation)
        .values(
            payment_id=payment.id,
        ...
        .returning(PaymentProviderOperation.id)
    )
    op_id = await session.scalar(stmt)
    return (
        await session.get(PaymentProviderOperation, op_id)
        if op_id
        else await session.scalar(
            select(PaymentProviderOperation).where(
                PaymentProviderOperation.idempotency_key == key
            )
        )
    )
...
async def retry_dead_provider_operation(
    ...
    if (
        op.operation_type == "create_payment"
        and not payment.external_id
        and now_utc() - op.created_at >= timedelta(hours=24)
    ):
    ...
    if reset_attempts:
        op.attempts = 0
    elif op.attempts >= op.max_attempts:
...
async def perform_http(claim, transport=YooKassaService):
    ...
    if claim.operation_type == "cancel_payment":
        result = await transport.cancel_payment_result(
            provider_id, idempotency_key=claim.idempotency_key
        )
    ...
    if result.ok:
        ...
        if status not in VALID_PROVIDER_STATUSES:
            invalid_snapshot_code = (
                ...
                if status is None
                else "provider_status_invalid"
            )
            result = classify_invalid_provider_snapshot(claim, data)
        ...
        if (
            result.ok
            and claim.operation_type == "cancel_payment"
        ...
        if result.ok:
            transition = await apply_provider_transition(
                session, payment, data, source=provider_transition_source(claim)
            )
            ...
            if (
                ...
                and transition.observed_status == "succeeded"
                ...
                for queued_op in queued:
                    queued_op.status = "cancelled"
                    queued_op.completed_at = now_utc()
        if result.ok:
            op.status = "succeeded"
            ...
            payment.reconciliation_status = (
                ...
                else payment.reconciliation_status
            )
    if not result.ok:
        op.last_error_code = (
            "cancel_not_confirmed"
            if locals().get("cancel_not_confirmed", False)
            else (
                locals().get("invalid_snapshot_code")
                or (result.error_kind.value if result.error_kind else "unknown")
            )
        )
        op.last_error = None
        exhausted = op.attempts >= op.max_attempts
        op.status = "dead" if exhausted or not result.retryable else "retry"
        ...
        if payment.provider_status in {"succeeded", "refunded", "canceled"}:
            payment.reconciliation_status = (
                "manual_review" if op.status == "dead" else "required"
            )
            session.add(
                PaymentEvent(
                    payment_id=payment.id,
                    event_type="provider_operation_error_after_terminal",
                    provider_status=payment.provider_status,
                    reason=op.last_error_code,
                    source="provider_finalizer",
                )
            )
        else:
            payment.provider_status = (
                ...
                if result.ambiguous
                    ...
                    "manual_review" if op.status == "dead" else payment.provider_status
                )
            )
            payment.reconciliation_status = (
                "required" if op.status == "retry" else "manual_review"
            )
        if op.status == "dead":
            op.completed_at = now_utc()
        if locals().get("cancel_not_confirmed", False) and op.status == "dead":
            ...
            session.add(
                PaymentEvent(
                    payment_id=payment.id,
                    event_type="cancel_not_confirmed_at_attempt_limit",
    ...
    for op in rows:
        ...
        if payment.provider_status in {"succeeded", "refunded", "canceled"}:
            ...
            session.add(
                PaymentEvent(
                    payment_id=payment.id,
                    event_type="stale_provider_operation_after_terminal",
                    provider_status=payment.provider_status,
                    reason="lease_expired_at_attempt_limit"
                    if dead
                    else "lease_expired",
...
async def finalize_provider_failure(session, claim, *, error_code, retryable):
    ...
    if payment.provider_status in {"succeeded", "refunded", "canceled"}:
        ...
        session.add(
            PaymentEvent(
                payment_id=payment.id,
                event_type="provider_failure_after_terminal",
                provider_status=payment.provider_status,
                reason=str(error_code)[:100],
                source="provider_failure_finalizer",
            )
        )
    elif dead:
        payment.provider_status = "manual_review"
