...
def validate_provider_payment(payment, data: dict) -> str | None:
    try:
        ...
        expected_amount = (
            ...
            if payment.snapshot_amount is not None
            else payment.amount
        )
        ...
        if Decimal(str(amount.get("value"))) != expected_amount:
            return "amount_mismatch"
        if str(amount.get("currency", "")).upper() != str(expected_currency).upper():
            return "currency_mismatch"
        if metadata.get("order_id") != payment.public_order_id:
            return "order_id_mismatch"
        local_id = metadata.get("local_payment_id")
        if local_id is None:
            return "local_payment_id_missing"
        if str(local_id) != str(payment.id):
