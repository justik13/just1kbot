...
CONCURRENCY=4; POLL_SECONDS=1.0
async def _claim(module,worker_id):
 async with session_scope() as session:
  await module.recover_stale(session); return await module.claim(session,worker_id)
 ...
 try:
  while not shutdown_event.is_set():
   ...
   for _ in range(CONCURRENCY-len(active)):
    module=queues[cursor%len(queues)]; cursor+=1; claim=await _claim(module,worker_id)
    if claim: active.add(asyncio.create_task(_run_claim(module,claim)))
   if active:
    done,_=await asyncio.wait(active,timeout=POLL_SECONDS,return_when=asyncio.FIRST_COMPLETED)
    for task in done:
     try: await task
     except asyncio.CancelledError: raise
   else:
    try: await asyncio.wait_for(shutdown_event.wait(),timeout=POLL_SECONDS)
    except asyncio.TimeoutError: pass
 except asyncio.CancelledError:
  for task in active: task.cancel()
  await asyncio.gather(*active,return_exceptions=True); raise
 finally:
  if active:
