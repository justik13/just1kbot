...
from database.repositories.paid_value_repo import (
    ...
    get_or_create_payment_reversal_entry,
)
...
async def claim(session, worker_id=None):
    ...
    op = await session.scalar(
        ...
        .where(
            ...
            PaymentFulfillmentOperation.next_attempt_at <= now_utc(),
            PaymentFulfillmentOperation.attempts
            < PaymentFulfillmentOperation.max_attempts,
        )
        .order_by(PaymentFulfillmentOperation.id)
    ...
    op.locked_at = now_utc()
    op.locked_by = worker_id
    op.attempts += 1
    await session.flush()
...
async def _entry(session, payment, user_id, typ, days, **kw):
    existing = await session.scalar(
        select(EntitlementEntry).where(
            ...
            EntitlementEntry.entry_type == typ,
        )
    )
    if existing:
        return existing, False
    item = EntitlementEntry(
        ...
        reversed_entry_id=kw.get("reversed"),
    )
    session.add(item)
    await session.flush()
    return item, True
...
async def grant(session, op):
    payment = await session.scalar(
        select(Payment).where(Payment.id == op.payment_id).with_for_update()
    )
    ...
    if (
        ...
        payment.fulfillment_last_error_code = "ineligible"
        op.status = "dead"
        op.completed_at = now_utc()
    ...
    existing = await session.scalar(
        select(EntitlementEntry).where(
            ...
            EntitlementEntry.source_type == "payment",
            EntitlementEntry.source_id == str(payment.id),
            EntitlementEntry.entry_type == grant_entry_type,
        )
    )
    ...
    if (
        not manual and payment.reconciliation_status in {"mismatch", "manual_review"}
    ) or refunded:
        payment.fulfillment_status = "manual_review"
        op.status = "cancelled"
        op.completed_at = now_utc()
    ...
    if not manual and (payment.tariff_quote_id or payment.tariff_version_id):
        if not payment.tariff_quote_id or not payment.tariff_version_id:
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "incomplete_quote_snapshot"
            op.status = "dead"
            op.completed_at = now_utc()
            return
        ...
        if (
            ...
            or payment.tariff_version_id != quote.target_tariff_version_id
            ...
            if quote:
                ...
                quote.diagnostic_reason = "payment_tariff_version_mismatch"
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "payment_tariff_version_mismatch"
            op.status = "dead"
            op.completed_at = now_utc()
            return
        if (
            payment.amount != payment.snapshot_amount
            or payment.amount != quote.confirmed_payment_required_rub
            or payment.currency != payment.snapshot_currency
            or payment.currency != quote.currency
            or (payment.snapshot_duration_days or 0) * 24 != version.duration_hours
            or payment.snapshot_device_limit != version.device_limit
        ):
            quote.status = "manual_review"
            quote.manual_review_at = quote.manual_review_at or now_utc()
            quote.diagnostic_reason = "quote_payment_snapshot_mismatch"
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "quote_payment_snapshot_mismatch"
            op.status = "dead"
            op.completed_at = now_utc()
            return
        if quote.status == "cancelled" or payment.checkout_status == "abandoned":
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "paid_after_checkout_cancel"
            op.status = "cancelled"
            op.completed_at = now_utc()
            return
        if quote.status not in {"active", "consumed", "expired"}:
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "quote_not_fulfillable"
            op.status = "cancelled"
            op.completed_at = now_utc()
            return
        if (
            payment.provider_confirmed_at is None
            or payment.provider_confirmed_at > quote.expires_at
        ):
            quote.status = "manual_review"
            quote.manual_review_at = quote.manual_review_at or now_utc()
            quote.diagnostic_reason = "provider_confirmation_missing_or_after_expiry"
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "quote_expired_at_confirmation"
            op.status = "cancelled"
            op.completed_at = now_utc()
            project_legacy_status(payment)
            return
        try:
            await get_or_create_confirmed_payment_entry(
                ...
                paid_value_rub=payment.snapshot_amount,
            )
        except PaidValueLedgerConflictError:
            quote.status = "manual_review"
            quote.manual_review_at = quote.manual_review_at or now_utc()
            quote.diagnostic_reason = "paid_value_ledger_conflict"
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "paid_value_ledger_conflict"
            op.status = "dead"
            op.completed_at = now_utc()
            return
    if not existing:
        days = payment.snapshot_duration_days
        if not days or not payment.snapshot_device_limit:
            payment.fulfillment_status = "manual_review"
            op.status = "dead"
            op.completed_at = now_utc()
            return
        ...
        await _entry(
            ...
            grant_entry_type,
            days,
            limit=payment.snapshot_device_limit,
            tariff=payment.tariff_id,
        )
    if quote is not None:
        if quote.status in {"active", "expired"}:
    ...
    await AuditService.log_action(
        ...
        details="durable fulfillment succeeded",
    )
    ...
    async def post_commit_success():
        ...
        await _notify_payment_success(
            user.telegram_id, tariff.name if tariff else "—", str(user.subscription_end)
        )
    ...
    if user.referred_by:
        await session.execute(
            ...
            .values(
                ...
                idempotency_key=f"payment-referral:{payment.id}",
                status="pending",
                payload={},
...
async def referral(session, op):
    ...
    if (
        ...
        or user.referred_by == user.telegram_id
        or payment.provider_status != "succeeded"
        or payment.fulfillment_status != "succeeded"
        or reversal
    ):
        op.status = "cancelled"
        op.completed_at = now_utc()
    ...
    if not ref:
        op.status = "dead"
        op.completed_at = now_utc()
        op.last_error_code = "referrer_missing"
        return
    if ref.is_deleted or ref.is_banned:
        op.status = "cancelled"
        op.completed_at = now_utc()
        return
    if (payment.snapshot_duration_days or 0) < 30:
        op.status = "cancelled"
        op.completed_at = now_utc()
        return
    marker = await session.scalar(
        select(ReferralReward)
        .where(ReferralReward.source_payment_id == payment.id)
        .with_for_update()
    )
    if marker and marker.reversed_at:
        op.status = "cancelled"
        op.completed_at = now_utc()
        return
    ...
    if eligibility and eligibility.status == "blocked":
        op.status = "cancelled"
        op.completed_at = now_utc()
        op.last_error_code = "referral_eligibility_blocked"
        return
    if not marker:
        ...
        if first:
            eligibility = ReferralEligibility(
                ...
                reason="first_durable_reward",
            )
            session.add(eligibility)
        marker = ReferralReward(
            referred_user_id=user.id,
            source_payment_id=payment.id,
        ...
        session.add(marker)
        await session.flush()
    user_days = 5 if marker.is_first else 0
    ref_days = 3 if marker.is_first else 1
    a = False
    if user_days:
        _, a = await _entry(session, payment, user.id, "referral_user_bonus", user_days)
    ...
    if b:
        await SubscriptionService.extend_subscription(
            session, ref.telegram_id, ref_days
        )
        ref.referral_days = (ref.referral_days or 0) + ref_days
    payment.referral_user_bonus_days = user_days
    payment.referral_referrer_bonus_days = ref_days
    op.status = "succeeded"
    op.completed_at = now_utc()
...
async def reverse(session, op):
    ...
    user_bonus = await session.scalar(
        select(EntitlementEntry).where(
            ...
            EntitlementEntry.source_type == "payment",
            EntitlementEntry.source_id == str(payment.id),
            EntitlementEntry.entry_type == "referral_user_bonus",
        )
    )
    ref_bonus = await session.scalar(
        select(EntitlementEntry).where(
            EntitlementEntry.source_type == "payment",
            EntitlementEntry.source_id == str(payment.id),
            EntitlementEntry.entry_type == "referral_referrer_bonus",
        )
    )
    if payment.referral_user_bonus_days > 0 and not user_bonus:
        payment.fulfillment_status = payment.reconciliation_status = "manual_review"
        payment.fulfillment_last_error_code = "referral_user_ledger_missing"
        op.status = "dead"
        op.completed_at = now_utc()
        project_legacy_status(payment)
        return
    if payment.referral_referrer_bonus_days > 0 and not ref_bonus:
        reason = (
            "legacy_referrer_unresolved"
            if payment.manual_review_reason == "legacy_referrer_unresolved"
            else "referral_referrer_ledger_missing"
        )
        payment.fulfillment_status = payment.reconciliation_status = "manual_review"
    ...
    expected_grant = bool(
        ...
        or payment.manual_review_reason
        in {"legacy_entitlement_snapshot_missing", "grant_ledger_missing"}
    )
    if not grant_entry and expected_grant:
        payment.fulfillment_status = "manual_review"
        payment.reconciliation_status = "manual_review"
        payment.fulfillment_last_error_code = "grant_ledger_missing"
        payment.manual_review_reason = "grant_ledger_missing"
        op.status = "dead"
        op.completed_at = now_utc()
        project_legacy_status(payment)
        return
    paid_entry = await session.scalar(
    ...
    if paid_entry is not None:
        try:
            await get_or_create_payment_reversal_entry(session, original=paid_entry)
        except PaidValueLedgerConflictError:
            payment.fulfillment_status = payment.reconciliation_status = "manual_review"
            payment.fulfillment_last_error_code = "paid_value_reversal_conflict"
            op.status = "dead"
            op.completed_at = now_utc()
            return
    if grant_entry and not reversal:
        days = abs(grant_entry.days_delta)
        now = now_utc()
        user.subscription_end = (
            max(now, user.subscription_end - timedelta(days=days))
        ...
        session.add(
            PaymentEvent(
                payment_id=payment.id,
                event_type="payment_reversal_no_grant",
                provider_status=payment.provider_status,
                reason="refund_before_fulfillment",
                source="fulfillment",
            )
        )
    referrals = (
        await session.scalars(
            ...
            .where(
                ...
                PaymentFulfillmentOperation.operation_type == "grant_referral",
                PaymentFulfillmentOperation.status.in_(("pending", "retry")),
            )
            .with_for_update()
        )
    ).all()
    for referral_op in referrals:
        referral_op.status = "cancelled"
        referral_op.completed_at = now_utc()
    ...
    if reward:
        reward.reversed_at = reward.reversed_at or now_utc()
    bonus_entries = (
        await session.scalars(
            select(EntitlementEntry).where(
                EntitlementEntry.source_type == "payment",
                EntitlementEntry.source_id == str(payment.id),
                EntitlementEntry.entry_type.in_(
                    ("referral_user_bonus", "referral_referrer_bonus")
                ),
            )
        )
    ).all()
    for bonus in bonus_entries:
        _, created = await _entry(
            session,
            payment,
            bonus.beneficiary_user_id,
            "referral_reversal",
            -bonus.days_delta,
            reversed=bonus.id,
            metadata={"reversed_entry_type": bonus.entry_type},
        )
        if created:
            beneficiary = await session.scalar(
                select(User)
                .where(User.id == bonus.beneficiary_user_id)
                .with_for_update()
            )
            if beneficiary and beneficiary.subscription_end:
                beneficiary.subscription_end = max(
                    now_utc(),
                    beneficiary.subscription_end - timedelta(days=bonus.days_delta),
                )
                if bonus.entry_type == "referral_referrer_bonus":
                    beneficiary.referral_days = max(
                        0, (beneficiary.referral_days or 0) - bonus.days_delta
                    )
    ...
    latest = await session.scalar(
        ...
        .order_by(Payment.fulfilled_at.desc())
        .limit(1)
    )
    if latest:
        user.device_limit = latest.snapshot_device_limit or user.device_limit
        user.current_tariff_id = latest.tariff_id
...
async def execute(session, claim):
    ...
    if not snapshot:
        raise PaymentFulfillmentOperationOwnershipError(claim.operation_id)
    await session.scalar(
        select(Payment).where(Payment.id == snapshot.payment_id).with_for_update()
    )
    op = await session.scalar(
        select(PaymentFulfillmentOperation)
        .where(PaymentFulfillmentOperation.id == claim.operation_id)
        .with_for_update()
    )
    if (
        not op
        or op.status != "processing"
        or op.locked_by != claim.worker_id
        or op.attempts != claim.attempt_number
    ):
        raise PaymentFulfillmentOperationOwnershipError(claim.operation_id)
    if op.operation_type == "grant_subscription":
        await grant(session, op)
    elif op.operation_type == "grant_referral":
        await referral(session, op)
    elif op.operation_type == "reverse_payment":
        await reverse(session, op)
    op.locked_at = op.locked_by = None
    await session.flush()
...
async def finalize_fulfillment_failure(session, claim, *, error_code, retryable=True):
    ...
    dead = (not retryable) or op.attempts >= op.max_attempts
    op.status = "dead" if dead else "retry"
    op.completed_at = now_utc() if dead else None
    op.next_attempt_at = now_utc() + timedelta(
        seconds=min(300, 2 ** min(op.attempts, 8))
    )
    op.last_error_code = str(error_code)[:100]
    op.last_error = None
    op.locked_at = op.locked_by = None
    if dead:
...
async def retry_dead_fulfillment_operation(
    session, operation_id, *, reset_attempts, reason
):
    op = await session.scalar(
        select(PaymentFulfillmentOperation)
        .where(PaymentFulfillmentOperation.id == operation_id)
        .with_for_update()
    )
    if not op or op.status != "dead":
        raise ValueError("operation is not dead")
    if not reset_attempts and op.attempts >= op.max_attempts:
        raise ValueError("reset_attempts required")
    op.status = "retry"
    op.completed_at = op.locked_at = op.locked_by = op.last_error_code = (
        op.last_error
    ) = None
    op.next_attempt_at = now_utc()
    op.payload = {**op.payload, "admin_retry_reason": str(reason)[:100]}
    if reset_attempts:
        op.attempts = 0
    return op
...
async def recover_stale(session, lease_seconds=FULFILLMENT_LEASE_SECONDS):
    rows = (
        await session.scalars(
            select(PaymentFulfillmentOperation)
            .where(
                PaymentFulfillmentOperation.status == "processing",
                PaymentFulfillmentOperation.locked_at
    ...
    for op in rows:
        dead = op.attempts >= op.max_attempts
        op.status = "dead" if dead else "retry"
        op.completed_at = now_utc() if dead else None
        op.locked_at = op.locked_by = None
        op.next_attempt_at = now_utc()
