...
#   1. AmneziaWG работает в Docker-контейнере и устанавливается/управляется
#      через официальный десктопный клиент AmneziaVPN (Self-hosted режим).
#   2. Контейнер Amnezia (amnezia-api / amnezia-awg) слушает локально на 127.0.0.1:4001.
#   3. Данный скрипт НЕ устанавливает AmneziaWG/DKMS на хост и НЕ трогает ядро Linux.
#   4. Скрипт настраивает ТОЛЬКО Nginx reverse proxy с SSL (Let's Encrypt)
#      на публичном порту (по умолчанию 8443), чтобы Telegram-бот just1kbot мог безопасно
#      обращаться к Amnezia REST API (AmneziaClient) по HTTPS.
#   5. Прямой доступ к ядру или awg syncconf со стороны бота запрещён и не используется.
#
# =============================================================================
# Использование:
...
#
# Что делает:
#   1. Проверяет что Amnezia API запущен в Docker на 127.0.0.1:4001
