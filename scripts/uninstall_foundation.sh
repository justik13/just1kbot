...
for module in "$SCRIPT_DIR/lib/uninstall_safe_core.sh" "$SCRIPT_DIR/lib/uninstall_safe_actions.sh"; do
    [[ -f "$module" && ! -L "$module" ]] || { printf 'ОШИБКА: uninstall module отсутствует или небезопасен: %s\n' "$module" >&2; exit 1; }
    # shellcheck source=/dev/null
    source "$module"
done
main(){
    parse "$@"; (( EUID==0 ))||fail 'запустите uninstall от root'; acquire_lock; manifest_preflight; read_env; confirm; prepare_postgres
    [[ "$MODE" == keep ]]&&backup_before_keep
    foundation_exists "$INSTALL_JOURNAL" || foundation_journal_begin uninstall preflight
    foundation_journal_update stopping-services; stop_units
    foundation_journal_update removing-nginx; remove_nginx; remove_certificate
    foundation_journal_update removing-database; purge_postgres
    foundation_journal_update removing-files; remove_files; purge_saved; remove_user
    post_verify
    if [[ "$MODE" == keep ]]; then
        # Keep a minimal manifest proving ownership of preserved PostgreSQL/backups.
        MANIFEST="$INSTALL_MANIFEST" python3 - <<'PY'
import json,os
from pathlib import Path
p=Path(os.environ['MANIFEST']); x=json.loads(p.read_text()); x['managed_resources']=[v for v in x['managed_resources'] if v.startswith('postgresql:') or v in ('path:/root/backups/just1kbot','path:/etc/just1kbot-backup.conf','path:/root/.config/just1kbot/backup.agekey')]; t=p.with_suffix('.tmp'); t.write_text(json.dumps(x,indent=2,sort_keys=True)+'\n'); t.chmod(0o600); t.replace(p)
PY
        chown root:root "$INSTALL_MANIFEST"; chmod 0600 "$INSTALL_MANIFEST"; foundation_journal_clear
        printf 'Приложение удалено; PostgreSQL и backups сохранены с residual ownership manifest.\n'
    else
        foundation_journal_clear; rm -f "$INSTALL_MANIFEST"; rmdir "$INSTALL_STATE_DIR" 2>/dev/null||true; rmdir "$STATE_ROOT" 2>/dev/null||true
