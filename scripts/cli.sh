...
cmd_update() {
    ...
    if [ "$update_ok" = "true" ]; then
        ...
        if [[ -n "$rollback_commit" ]]; then
            ...
            log "Откат кода завершён. Предыдущая версия сервисов запущена."
            warn "Примечание: если применённая миграция изменила схему БД и несовместима со старой версией,"
            warn "выполните восстановление созданного перед обновлением бэкапа: just1kbot restore"
        fi
        return 1
    fi
