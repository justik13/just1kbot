...
Вся информация верифицирована, синхронизирована с кодовой базой и официальными источниками для автономной разработки без необходимости поиска в интернете.
...
* **Назначение:** Полная спецификация протокола AmneziaWG 2.0 (`amneziawg2`), форматы файлов `.conf` и `.vpn`, схема URI `vpn://...`, обфускационные параметры (`Jc`, `Jmin`, `Jmax`, `S1-S4`, `H1-H4`, `I1-I5` / `h1-h5`), API-эндпоинты `kyoresuas/amnezia-api` и чеклист валидации.

### 2. [INCY Application & Subscription Protocol](incy_docs.md)
* **Назначение:** Спецификация формата подписки INCY, протокольные схемы (`amneziawg://`, `wireguard://`, `vless://`), правила кодирования Base64URL, стандартные HTTP-заголовки управления (`profile-title`, `subscription-userinfo`, `announce`), deep links (`incy://import/`, `incy://crypt1/`) и кросс-платформенная матрица поддержки (iOS, Android vs Desktop).
