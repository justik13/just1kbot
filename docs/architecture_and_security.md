...
| **Шифрование** | `cryptography` (MultiFernet, HMAC-SHA256, age) | Защита конфиденциальных данных в БД (MultiFernet) и дампов (age) |
...
* Поля `raw_config`, `api_key` и другие чувствительные данные шифруются на уровне приложения с помощью типа данных `EncryptedString` (Fernet / MultiFernet: AES-128-CBC + HMAC-SHA256).
...
## 🎨 5. TELEGRAM UI & MESSAGE EFFECTS SPECIFICATION

Параметр `message_effect_id` в Telegram Bot API ограничен сервером исключительно эффектами без флага `premium_required: true`. По результатам валидации всех 696 MTProto-эффектов, Telegram Bot API поддерживает ровно 6 эффектов:

| Эмодзи | Название константы | Точный 64-битный ID | Назначение в архитектуре Just1kBot |
|---|---|---|---|
| 👍 | `EFFECT_LIKE` | `5107584321108051014` | Системные подтверждения, переименование устройства |
| 👎 | `EFFECT_DISLIKE` | `5104858069142078462` | Отмена операций, сброс состояний |
| ❤️ | `EFFECT_HEART` | `5159385139981059251` | Отзывы, бонусы, благодарности |
| 🔥 | `EFFECT_FIRE` | `5104841245755180586` | Успешное создание VPN-устройства (`EFFECT_LIGHTNING`), реферальные начисления |
| 🎉 | `EFFECT_CONFETTI` | `5046509860389126442` | Пополнение баланса, покупка / продление тарифа |
| 💩 | `EFFECT_POOP` | `5046589136895476101` | Критические ошибки, предупреждения |
