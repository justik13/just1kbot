...
В сообществе иногда возникает соблазн упростить схему («зачем два сервера, если можно настроить 1-Hop: `CDN ➔ Exit за рубежом напрямую`?»). Однако в эксплуатационной модели сервиса **Just1kbot** отказ от промежуточного Origin-сервера в РФ сопряжен с **многократным ростом эксплуатационного риска компрометации зарубежного Exit-узла**.
...
| **Вердикт для сервиса Just1kbot** | ❌ Не рекомендуется для production (высокий риск потери нод) | 🏆 **Обязательный производственный стандарт (SSOT)** |
...
5. **Beeline CDNvideo и CDN с фильтрацией статических расширений (Статус PR #6307 vs PR #6720):**
   ...
   - **Техническое разграничение официального функционала (PR #6307) и экспериментальных патчей (PR #6720):**
     1. **Официальный релиз (Stock Xray-core v26.7.28+ на базе PR #6307):**
        ...
        ```
        Это полностью и без модификаций бинарника решает проблему фильтрации расширений на любых CDN в рамках официального стабильного релиза (рекомендованный стандарт для Just1kbot).
