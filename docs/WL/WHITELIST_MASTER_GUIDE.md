...
### ЧАСТЬ I. ИНЖЕНЕРНО-ТЕХНИЧЕСКИЙ СПРАВОЧНИК ПО БЕЛЫМ СПИСКАМ (WHITELIST MASTER GUIDE)
1. [Введение и фундаментальная разница: черные vs белые списки](#1-введение-и-фундаментальная-разница-черные-vs-белые-списки)
   - [1.1. Защита Exit-узлов: почему в production необходим каскад 2-Hop](#11-защита-exit-узлов-почему-в-production-необходим-каскад-2-hop)
...
### ЧАСТЬ II. АРХИТЕКТУРНЫЕ ИДЕИ И ЭКСПЛУАТАЦИОННЫЕ ЗАМЕТКИ ДЛЯ JUST1KBOT
11. [Концепт строгой изоляции контуров в Just1kbot](#11-концепт-строгой-изоляции-контуров-в-just1kbot)
12. [Архитектурная идея 2-Hop каскада для серверного парка Just1kbot](#12-архитектурная-идея-2-hop-каскада-для-серверного-парка-just1kbot)
13. [Моделирование экономики и тарификации для пользователей бота](#13-моделирование-экономики-и-тарификации-для-пользователей-бота)
14. [Концепт клиентской выдачи подписок через Telegram-интерфейс](#14-концепт-клиентской-выдачи-подписок-через-telegram-интерфейс)
...
> Настоящий документ служит **практическим инженерным справочником по обходу белых списков (Default-Drop) в РФ**, систематизируя производственный опыт проектирования, развертывания и масштабирования высоконагруженных прокси-инфраструктур.
...
### 1.1. Защита Exit-узлов: почему в production необходим каскад 2-Hop

В схеме 1-Hop (`CDN ➔ Exit за рубежом напрямую`) зарубежный сервер быстро попадает под блокировку. Промежуточный Origin-сервер в РФ изолирует зарубежный узел от прямого обнаружения.

#### 1. Эксплуатационные риски схемы 1-Hop (CDN ➔ Exit напрямую):

1. **Аномальный трансграничный поток от пула CDN (Cross-Border CDN Egress Anomaly):**  
   Когда сотни абонентов прокачивают интенсивный трафик, Anycast-ноды отечественного CDN вынуждены напрямую концентрировать весь исходящий международный поток на единичных зарубежных IP-адресах Exit-серверов. Концентрация трансграничного egress-трафика от распределенной инфраструктуры CDN на зарубежном хостинге привлекает внимание автоматизированных эвристик мониторинга магистральных стыков.
2. **Отсутствие изоляции сетевого периметра (Раскрытие Exit-узла наружу):**  
   В схеме 1-Hop зарубежный Exit-сервер вынужден держать открытым входящий порт для всех динамических подсетей CDN (а без строгой фильтрации — для всего интернета). Это существенно расширяет поверхность атаки для сканеров Censys, Shodan и систем активного сетевого зондирования (Active Probing).
3. **Уязвимость перед антифродом мобильных приложений РФ:**  
   Если в схеме отсутствует экранирование Origin-узлом, а клиентское устройство временно не использует Per-App Split Tunneling, приложения с банковскими и антифрод-SDK (F.A.C.C.T. / Kaspersky / Сбер) фиксируют зарубежный IP узла выхода и вносят его в базы мониторинга.
4. **Невозможность фильтрации паразитного трафика:**  
   Без промежуточного Nginx в РФ невозможно применить Mobile-Only Geo-Filter по автономным системам (ASN). Домашние абоненты на проводных провайдерах и Wi-Fi создают паразитный трафик, стремительно исчерпывая лимиты CDN и создавая аномальный профиль нагрузки.

#### 2. Почему каскад 2-Hop (CDN ➔ Origin РФ ➔ Exit Зарубежье) защищает production-инфраструктуру:
...
1. **Принцип «Одноразового бронежилета» (Disposable Bastion Shield):**  
   ...
   ```
   Ни единый пакет из внешнего мира, кроме единственного доверенного IP Origin-сервера в РФ, физически не может достучаться до порта Exit-узла. Сканеры Shodan/РКН видят порт как абсолютно недоступный (`filtered`).
...
3. **Устойчивость к компрометации:**  
   Даже если Origin-сервер в РФ когда-либо попадет под подозрение, замена промежуточного узла в Москве занимает считанные минуты. При этом **дорогостоящий, сложно настроенный Exit-сервер за рубежом (со всеми базами, WARP, маршрутизацией и пулом белых IP) надежно экранирован фаерволом от прямого входящего сканирования Censys/Shodan/DPI и полностью изолирован от непосредственного взаимодействия с CDN**.
...
#### 3. Сравнительная матрица: 1-Hop vs 2-Hop

| Характеристика | Схема 1-Hop (CDN ➔ Exit напрямую) | Каскад 2-Hop (CDN ➔ Origin РФ ➔ Exit) — Рекомендованный production-стандарт |
| :--- | :--- | :--- |
| **Риск компрометации зарубежного Exit IP** | **Высокий** (прямой трансграничный egress пула CDN, открытый порт наружу) | **Минимальный** (Exit изолирован UFW, трафик поступает строго с IP Origin) |
...
| **Риск превышения 5s таймаута CDN** | Повышенный (при задержках трансграничных магистралей) | **Минимальный** (RTT между CDN Edge и Origin в РФ <3 мс) |
| **Инженерный вердикт** | ❌ Не рекомендуется для production (высокий риск потери нод) | 🏆 **Обязательный производственный стандарт (SSOT)** |
...
5. **Beeline CDNvideo и CDN с фильтрацией статических расширений (Статус PR #6307 vs PR #6720):**
   ...
   - **Техническое разграничение официального функционала (PR #6307) и экспериментальных патчей (PR #6720):**
     1. **Официальный релиз (Stock Xray-core v26.7.28+ на базе PR #6307):**
        В ранних версиях Xray транспорт XHTTP при передаче подпутей сессий/потоков принудительно добавлял завершающий слэш (`/`), превращая путь в `/assets/app.js/`, что ломало сопоставление статических расширений на edge-нодах CDN. В официальный релиз Xray v26.7.28 вошел PR #6307, снявший требование обязательного закрывающего слэша при передаче метаданных сессии в HTTP-заголовках или строке запроса. Благодаря этому в стоковом официальном Xray на клиенте и сервере задается чистый фиксированный путь со статическим расширением:
        ```json
        "xhttpSettings": {
          "path": "/assets/app.js",
          "mode": "packet-up"
        ...
        ```
        Это полностью и без модификаций бинарника решает проблему фильтрации расширений на любых CDN в рамках официального стабильного релиза (рекомендованный стандарт для стабильных production-конфигураций).
     2. **Экспериментальный патч динамической рандомизации (Xray-core PR #6720, статус: closed unmerged):**
        В PR #6720 была предложена расширенная клиентская генерация псевдослучайных URL для каждого HTTP-запроса (например, `/assets/7f3a9e.js`):
        ```json
        "xhttpSettings": {
          "path": "/assets/",
          "pathTable": "hex",
        ...
        ```
        > [!IMPORTANT]
        > **Разграничение статуса:** PR #6720 был закрыт мейнтейнерами Xray без слияния в основную ветку. Параметры `pathTable`, `pathLength` и `pathExtension` **не входят** в официальные сборки Xray-core и поддерживаются только кастомными патченными клиентами. Для надежной production-эксплуатации рекомендуется использовать официальный механизм чистых путей из п. 1 (PR #6307).
...
# Доверенный периметр CDN: заголовок X-Real-IP принимается ТОЛЬКО от проверенных адресов Yandex Cloud CDN.
# Любые прямые запросы от клиентов со спуфингом заголовка X-Real-IP будут проигнорированы.
# Официальный пул публичных префиксов edge/egress Yandex Cloud (https://yandex.cloud/ru/docs/security/ip-list):
set_real_ip_from 213.180.193.0/24;
set_real_ip_from 213.180.204.0/24;
set_real_ip_from 87.250.250.0/24;
set_real_ip_from 87.250.251.0/24;
set_real_ip_from 77.88.44.0/24;
set_real_ip_from 77.88.55.0/24;
set_real_ip_from 93.158.134.0/24;
set_real_ip_from 178.154.200.0/24;
set_real_ip_from 178.154.205.0/24;
set_real_ip_from 84.201.128.0/18;
set_real_ip_from 178.154.224.0/20;
set_real_ip_from 130.193.32.0/19;
set_real_ip_from 51.250.0.0/16;
set_real_ip_from 158.160.0.0/16;
...
# Валидация полноты выгрузки: защита от перезаписи файла при сбоях RIPEstat API
PREFIX_COUNT=$(grep -c ' 1;' "$TMP_CONF" || true)
if [ "$PREFIX_COUNT" -lt 500 ]; then
    echo "[ERROR] Only $PREFIX_COUNT prefixes received from RIPEstat (minimum threshold is 500)! Aborting update to protect active allowlist." >&2
    rm -f "$TMP_CONF"
    exit 1
fi

# Сохранение резервной копии рабочей конфигурации
if [ -f "$OUTPUT_CONF" ]; then
    cp -f "$OUTPUT_CONF" "${OUTPUT_CONF}.bak"
fi
...
if nginx -t > /dev/null 2>&1; then
    nginx -s reload
    echo "[OK] Mobile ranges successfully updated ($PREFIX_COUNT prefixes) and Nginx reloaded."
else
    echo "[ERROR] Invalid Nginx configuration generated! Rolling back to backup..." >&2
    if [ -f "${OUTPUT_CONF}.bak" ]; then
        mv "${OUTPUT_CONF}.bak" "$OUTPUT_CONF"
...
```nginx
# Вне блока server {} задаем полный доверенный периметр CDN Yandex Cloud:
# Публичные диапазоны Anycast/Egress узлов Yandex Cloud CDN (https://yandex.cloud/ru/docs/security/ip-list)
set_real_ip_from 213.180.193.0/24;
set_real_ip_from 213.180.204.0/24;
set_real_ip_from 87.250.250.0/24;
set_real_ip_from 87.250.251.0/24;
...
> [!TIP]
> **Сетевая изоляция порта 443 на Origin через UFW:**
> Для максимального ужесточения сетевого периметра порт 443 на Origin-сервере можно ограничить на уровне ядра Linux (UFW), разрешив входящие подключения исключительно от доверенных подсетей edge-нод Yandex Cloud CDN:
> ```bash
> # Разрешение трафика на 443 порт Origin строго из пула Yandex Cloud CDN
> for cidr in 213.180.193.0/24 213.180.204.0/24 87.250.250.0/24 87.250.251.0/24 \
>             77.88.44.0/24 77.88.55.0/24 93.158.134.0/24 178.154.200.0/24 \
>             178.154.205.0/24 84.201.128.0/18 178.154.224.0/20 130.193.32.0/19 \
>             51.250.0.0/16 158.160.0.0/16; do
...
3. **Репозитории и исходные коды:**
   ...
   - [XTLS/Xray-core PR #6307: Снятие принудительного trailing slash в XHTTP (поддержка чистых путей со статическими расширениями в stock v26.7.28+)](https://github.com/XTLS/Xray-core/pull/6307)
   - [XTLS/Xray-core PR #6720: Динамическая маскировка XHTTP под статические файлы (pathTable, pathLength, pathExtension — closed unmerged)](https://github.com/XTLS/Xray-core/pull/6720)
...
> [!IMPORTANT]
> **ПРАВОВОЙ И ИНЖЕНЕРНЫЙ ДИСКЛЕЙМЕР РАЗДЕЛА (STATUS: IDEAS & CONCEPTS ONLY):**
> Материалы части II представляют собой **исследовательские идеи, концептуальные гипотезы и архитектурные заметки** по возможному развитию проекта `Just1kbot`.
> Настоящий раздел:
> 1. **НЕ ЯВЛЯЕТСЯ призывом к немедленному действию**, инструкцией к переписыванию кодовой базы или обязательным требованием.
> 2. **НЕ УТВЕРЖДАЕТ, что текущая кодовая база уже устроена именно так** или содержит дефекты. Это концептуальное проектирование «на вырост» для оценки масштабируемости и устойчивости.
> 3. Любое возможное внедрение описанных здесь решений требует отдельного предварительного согласования, технико-экономического обоснования и явного одобрения владельца проекта (`APPROVE`).

---

## 11. Концепт строгой изоляции контуров в Just1kbot

### 11.1. Принцип изоляции протокольных флотов
В мультипротокольной платформе нельзя смешивать протоколы в одном серверном пуле или в одном клиенте.

```
┌────────────────────────────────────────────────────────────────────────┐
│                   JUST1KBOT: СТРОГАЯ ИЗОЛЯЦИЯ КОНТУРОВ                  │
├──────────────────────────────────┬─────────────────────────────────────┤
│      КОНТУР А: СТАНДАРТ (AWG)    │      КОНТУР Б: БЕЛЫЙ ИНТЕРНЕТ (WL)   │
├──────────────────────────────────┼─────────────────────────────────────┤
│ Протокол: AmneziaWG (awg/udp)    │ Протокол: Xray VLESS XHTTP (tcp/tls)│
│ Клиентский софт: AmneziaVPN /    │ Клиентский софт: строго INCY        │
│   AmneziaWG / DEFAULTVPN         │ Серверный пул: origin.protocol=xray │
│ Серверный пул: origin.protocol=awg│ Формат выдачи: строго Sub URL (sub) │
│ Бэкенд-клиент: AmneziaClient     │ Бэкенд-клиент: XrayNodeClient       │
└──────────────────────────────────┴─────────────────────────────────────┘
```

1. **Техническое обоснование:**
   - AmneziaWG передает UDP-датаграммы с обфусцированным заголовком (`Jc`, `Jmin`, `Jmax`, `S1`, `S2`, `H1`-`H4`). Российские CDN (включая Yandex Cloud CDN) работают как обратные L7 HTTP-прокси и не маршрутизируют UDP. Пропустить AWG через CDN технически невозможно.
   - Xray VLESS XHTTP передает потоки HTTP/2 через TLS на порт 443. Это работает через L7 CDN Anycast, но требует отдельного пула сертификатов и управления сессиями.
2. **Изоляция в базе данных и серверном пуле:**
   - Серверы жестко типизируются по атрибуту протокола (`Server.protocol = "awg"` vs `Server.protocol = "xray"`).
   - Запросы на выдачу конфигураций или назначение нод фильтруются строго по целевому протоколу, исключая случайный кросс-роутинг.
3. **Изоляция клиентов и форматов:**
   - Для пользователей стандартного тарифа выдаются конфигурационные файлы AWG (`.conf`) или ключи для приложений экосистемы WireGuard/Amnezia.
   - Для режима Белого Интернета выдается исключительно защищенный URL подписки (`sub`), предназначенный для клиента INCY. Выдача сырых VLESS-ключей не рекомендуется во избежание десинхронизации параметров при ротации CDN-ресурсов или смене путей.

---

## 12. Архитектурная идея 2-Hop каскада для серверного парка Just1kbot

### 12.1. Концепция буферных Origin-нод в РФ
Двухуровневый каскад (2-Hop) защищает зарубежные Exit-ноды (Нидерланды, Германия, Финляндия) от прямых блокировок ТСПУ:

```
Пользователь (РФ, LTE)
       │ HTTPS / SNI: cdn.domain.ru (Метод OPTIONS)
       ▼
[ Yandex Cloud CDN (Anycast Edge) ]
       │ HTTPS / 443 (Пересылка запроса на чистый Origin)
       ▼
[ Origin VPS (РФ, Москва / СПб) ] ── (Nginx: rewrite OPTIONS -> POST, Zero Buffering)
       │                                  │
       │                                  ▼
       │                            [ Xray-core (Inbound XHTTP) ]
       │                                  │
       │ VLESS over Reality / TLS         │ (Outbound VLESS)
       └──────────────────────────────────┘
                      │
                      ▼
       [ Exit VPS (Нидерланды / Германия) ] ── (Cloudflare WARP)
                      │
                      ▼
              Свободный Интернет
```

### 12.2. Преимущества архитектуры для флота Just1kbot
1. **Защита Exit-серверов от сетевого бана:**
   - Российские ТСПУ и автоматические сканеры РКН видят только обращение мобильных клиентов к доверенным Anycast IP Yandex Cloud CDN.
   - Origin-сервер в РФ видит входящие запросы только от CDN (IP-диапазоны Yandex Cloud).
   - Зарубежные Exit-ноды принимают подключения только с фиксированных IP-адресов российских Origin-серверов (по белому списку в `ufw` / `iptables`).
   - Зарубежные IP-адреса сервиса не фигурируют в трафике российских абонентов и не попадают под анализ ТСПУ.
2. **Контроль входящего трафика через Mobile-Only Geo-Filter:**
   - Nginx на Origin-сервере проверяет клиентские IP через RIPEstat API (паттерн `zxcstas/yandex-cdn-mobile-only`).
   - Если запрос пришел не от мобильных операторов (МТС AS8359, МегаФон AS25159, Билайн AS16345, Т2 AS12958), Nginx отдает статическую страницу-заглушку (Cover-Site), чтобы боты и сканеры не расходовали квоты CDN.

---

## 13. Моделирование экономики и тарификации для пользователей бота

### 13.1. Анализ пропускной способности Yandex Cloud CDN: 100–150 Мбит/с (эмпирические замеры сообщества)
> [!NOTE]
> Приведённые скоростные показатели являются результатами эмпирических сетевых замеров сообщества в тестовых стендах (неофициальные данные), а не документированными SLA провайдера CDN.

Потолок 100–150 Мбит/с действует на одну сессию одного клиента, а не на суммарный канал CDN:

| Параметр | Одиночный клиент (Single Stream) | Вся инфраструктура (Aggregate Throughput) |
| :--- | :--- | :--- |
| Реальный предел | 100–150 Мбит/с (практический потолок) | 1 Гбит/с и более (масштабируется линейно) |
| Причина ограничения | RTT, окно HTTP/2 Flow Control (RFC 7540), упаковка в packet-up | Сеть CDN Anycast распределяет нагрузку на десятки Edge-нод в РФ |
| Узкое место | Клиентский стек TCP, радиоканал мобильной связи | Пропускная способность портов Origin VPS и Exit VPS |

Лимит 100–150 Мбит/с касается одной сессии одного пользователя: в режиме `packet-up` трафик нарезается на чанки поверх HTTP/2. При блокировках скорости 30–80 Мбит/с хватает для веб-страниц, голосовой связи и видео 1080p.
Суммарная емкость CDN для всех абонентов не ограничена 150 Мбит/с: 50 активных клиентов суммарно передают 500–800 Мбит/с через Edge-ноды CDN без взаимных помех.

### 13.2. Расчет квот и тарификация
Правила тарификации Yandex Cloud CDN включают:
1. **150 ГБ исходящего трафика (Egress) в месяц** на один ресурс (включено в базовую подписку ресурса 150 ₽/месяц; входящий трафик Ingress на CDN — **0 ₽**, бесплатный).
2. **100 000 000 бесплатных HTTP-запросов в месяц** на каждый CDN-ресурс.
3. Сверх лимита тарифицируется только исходящий трафик (Egress в РФ — **1.054 ₽ / ГБ** с НДС).

**Оценка расхода запросов при VLESS XHTTP (packet-up):**
- В режиме `packet-up` исходящий трафик клиента разбивается на отдельные HTTP-запросы методом `OPTIONS`.
- При активном веб-серфинге один пользователь генерирует в среднем от 10 000 до 30 000 HTTP-запросов в час активности.
- При типовом профиле использования (2–3 часа активности в день) один абонент генерирует порядка 1.5–3 млн запросов в месяц.
- Пул в 100 млн запросов позволяет комфортно обслуживать постоянную базу из 30–50 активных пользователей в месяц без превышения порога бесплатных запросов ресурса.
- Режим высокой доступности рассчитан на аварийный доступ к сайтам и мессенджерам с квотой 20–50 ГБ в месяц на пользователя. Он не предназначен для торрентов или непрерывного 4K-стриминга.

---

## 14. Концепт клиентской выдачи подписок через Telegram-интерфейс

### 14.1. Соблюдение комплаенс-правил проекта
В интерфейсе Telegram-бота (кнопки, тексты, уведомления, карточки тарифов) запрещены стоп-слова: `VPN`, `ВПН`, `обход блокировок`, `белые списки`, `глушилки`, `LTE`.
- Нейтральные формулировки для UI:
  - «Специальный режим подключения (INCY)»
  - «Резервный протокол доступа»
  - «Профиль высокой доступности»
  - «Универсальный профиль для мобильных сетей»

### 14.2. Архитектура эндпоинта подписки
Бэкенд бота отдает конфигурацию для INCY через защищенный HTTP-эндпоинт:
`GET https://domain.com/sub/wl/{secure_user_token}`

1. **Служебные HTTP-заголовки ответа (INCY Management Headers):**
   ```http
   HTTP/1.1 200 OK
   Content-Type: text/plain; charset=utf-8
   Profile-Title: Just1k - Резервный режим
   Profile-Update-Interval: 12
   Subscription-Userinfo: upload=1073741824; download=5368709120; total=53687091200; expire=1780000000
   ```
   - `Profile-Title`: имя профиля в приложении INCY.
   - `Profile-Update-Interval`: интервал автообновления профиля клиентом в часах (каждые 12 часов).
   - `Subscription-Userinfo`: биллинговая телеметрия (трафик, лимит, дата окончания).

2. **Тело ответа подписки:**
   - Base64-закодированный список VLESS-ссылок или Full Xray JSON-структура.
   - VLESS URI формируется с учетом чистых путей и метода OPTIONS:
     ```
     vless://{UUID}@{CDN_CNAME}:443?encryption=none&security=tls&sni={CDN_CNAME}&type=xhttp&path=%2Fassets%2Fapp.js&mode=packet-up&extra=%7B%22uplinkHTTPMethod%22%3A%22OPTIONS%22%7D#{TAG_URL_ENCODED}
     ```

### 14.3. Однокликовые Deep Links для Telegram-бота
Кнопки быстрого импорта в боте:
1. **Ссылка на подписку:**
   `https://t.me/share/url?url=...` или прямая ссылка `incy://add/https://domain.com/sub/wl/{token}`.
2. **Зашифрованная конфигурация:**
   Формат `incy://crypt1/...` для импорта профиля в один клик без QR-кодов.

### 14.4. Сводная таблица сравнения концептов реализации

| Характеристика | Базовый сценарий (Текущий baseline) | Перспективный концепт каскада 2-Hop |
| :--- | :--- | :--- |
| **Основной протокол** | AmneziaWG (UDP) | Двухконтурный: AWG (Обычный) + Xray XHTTP (Белые списки) |
| **Поведение при Default-Drop** | Полная недоступность (UDP дропается) | Стабильная работа через Anycast CDN (TCP/443 OPTIONS) |
| **Клиентское приложение** | AmneziaVPN / AmneziaWG / DEFAULTVPN | AmneziaVPN (для AWG) + INCY (для Xray XHTTP) |
| **Топология сети** | Прямой туннель: Клиент -> Зарубежная нода | Каскад: Клиент -> CDN -> Origin (РФ) -> Exit (EU) |
| **Безопасность зарубежных IP** | IP ноды открыт и может быть заблокирован ТСПУ | IP Exit-ноды полностью скрыт от РФ за Origin-сервером |
| **Сложность инфраструктуры** | Низкая (один сервер на локацию) | Средняя (требуется координация CDN + Origin в РФ) |
