...
1. [Введение и фундаментальная разница: черные vs белые списки](#1-введение-и-фундаментальная-разница-черные-vs-белые-списки)
   - [1.1. Защита от оверинженеринга: минималистичная схема 1-Hop vs каскадная 2-Hop](#11-защита-от-оверинженеринга-минималистичная-схема-1-hop-vs-каскадная-2-hop)
...
### 1.1. Защита от оверинженеринга: минималистичная схема 1-Hop vs каскадная 2-Hop

Перед развертыванием инфраструктуры критически важно провести инженерную оценку потребностей по принципу «Бритвы Оккама» (не плодить сущности сверх необходимого). В сообществе часто воспроизводят максимально сложную архитектуру без понимания того, кому она реально нужна.

#### 1. Сравнительная матрица: 1-Hop vs 2-Hop

| Критерий | Вариант 1: Минималистичный 1-Hop (CDN ➔ Exit напрямую) | Вариант 2: Полный каскад 2-Hop (CDN ➔ Origin РФ ➔ Exit) |
| :--- | :--- | :--- |
| **Целевая аудитория** | **Личное использование, семья, малая группа (1–5 человек)** | **Публичный / полупубличный VPN-сервис (десятки/сотни пользователей)** |
| **Количество серверов** | **1 VPS за рубежом** (Германия, Финляндия, Нидерланды) | **2 VPS** (1 в РФ для Origin + 1 за рубежом для Exit) |
| **Затраты на хостинг** | Минимальные (~300–500 ₽/мес за один зарубежный VPS) | Удвоенные (~800–1200 ₽/мес за два VPS) |
| **Сложность настройки** | Низкая: CDN указывает прямо на `exit.example.com:443` | Высокая: Nginx, cron, RIPEstat API, UFW, два инстанса Xray |
| **Точки отказа** | 2 точки: CDN и Exit-сервер | 3 точки: CDN, Origin в РФ и Exit-сервер |
| **Защита квоты 150 ГБ** | Ручная: выключение VPN на домашнем Wi-Fi или клиентские правила | Автоматическая: Nginx Mobile-Only Geo-Filter по ASN |
| **Риск таймаута CDN (5s)** | Возможен, если пинг от Edge до Exit >80 мс при медленном TLS | Минимален: Origin в Москве обеспечивает RTT <3 мс к Edge |

#### 2. Где кроется типичный оверинженеринг (Overengineering Traps)

1. **Развертывание Origin в РФ для 1–2 человек:**
   Yandex Cloud CDN позволяет настроить в качестве источника (Origin) любой сервер в мире, включая зарубежный VPS с Xray! Если ваш Exit-сервер имеет хорошую сетевую связность (RTT до Москвы <45–50 мс), промежуточный Origin в РФ является **избыточным усложнением**. Начните с прямого подключения `CDN ➔ Exit`.
2. **Скрипты Mobile-Only парсинга RIPEstat:**
   Парсинг 9 ASN и генерация 10 000+ префиксов Nginx нужны исключительно сервисам с неконтролируемыми пользователями. Для личного смартфона достаточно правила в клиенте: «Bypass Wi-Fi» или ручное отключение туннеля дома.
3. **Multi-CDN Failover с тремя параллельными облаками:**
   Настройка Yandex Cloud + Selectel + VK Cloud одновременно требует трех аккаунтов, трех биллингов и сложной Observatory. Для 99% задач достаточно одного надежного CDN (YC) и одного запасного профиля прямого подключения (VLESS Reality) на случай форс-мажора.
4. **Cloudflare WARP на Exit-сервере:**
   Интеграция WARP с кастомным MTU 1280 и TCPMSS Clamping требуется **только если** хостер вашего Exit-узла заблокирован сервисами OpenAI или Google. Если IP-адрес чистый — WARP избыточен и лишь добавляет задержку.
