...
5. **Beeline CDNvideo и CDN с фильтрацией статических расширений (XTLS/Xray-core PR #6720):**
   ...
   - **Инженерные решения для прохождения фильтра расширений:**
     1. **Стандартный способ (Stock Xray v26.7.28):** Задание фиксированного пути с расширением статического файла в клиентской и серверной конфигурациях (стало доступно после #6307 благодаря удалению принудительного закрывающего слэша):
        ```json
        "xhttpSettings": {
          "path": "/assets/app.js",
          "mode": "packet-up"
        ...
        ```
     2. **Динамическая маскировка (Xray-core PR #6720, сентябрь 2026):**
        В PR #6720 предложен механизм рандомизации URL под статические файлы на клиенте:
        ```json
        "xhttpSettings": {
          "path": "/assets/",
          "pathTable": "hex",
        ...
        ```
        Клиент с поддержкой PR #6720 генерирует URL вида `GET /assets/61877619ff3cadff.js?v=...&t=...`. Сервер Xray на Origin (включая стоковый непатченный `v26.7.28`) сопоставляет путь по префиксу `strings.HasPrefix` и успешно обрабатывает запрос без необходимости изменений на стороне сервера.
...
- **Почему именно метод `OPTIONS` для Yandex Cloud CDN?**
  ...
  - *Эмпирическое наблюдение на ТСПУ:* По данным сетевых замеров операторов РФ, запросы `OPTIONS` не вызывают подозрения эвристических анализаторов DPI, так как стандартно генерируются браузерами в качестве предварительных служебных CORS preflight-запросов перед загрузкой веб-страниц.
...
# Верификация целостности бинарного архива перед установкой:
...
# Корректное извлечение клиентского IP от CDN с fallback на $remote_addr
...
```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ ПОЛЬЗОВАТЕЛЬСКАЯ ПЛОСКОСТЬ (USER DATA PLANE — 100% REMOTE ENCRYPTED DNS)              │
│                                                                                        │
│ [Смартфон / Ноутбук]                                                                  │
...
#### 3. Нулевая вероятность DNS-утечки (DNS Leak Proof)
В правильно сконфигурированном клиенте (INCY / Amnezia / sing-box) отправка DNS-запросов через мобильного оператора РФ блокируется на уровне ядра туннеля:
1. Виртуальный сетевой адаптер перехватывает стандартный порт 53 (UDP/TCP) и направляет весь DNS-трафик в стек Xray.
2. Встроенный DNS-маршрутизатор Xray выполняет резолвинг строго через защищенный протокол DoH (DNS over HTTPS) через удаленный Exit-узел.
3. ТСПУ и провайдер мобильной связи видят только непроницаемый TLS-трафик к разрешенному хосту CDN. Попытки DNS-спуфинга (подмены IP-адресов цензорами) математически невозможны из-за криптографической проверки сертификата DoH.
