...
| **Xray** (`vless` over XHTTP) | ✅ **PRODUCTION (Белый Интернет)** | Реализован в контуре «Белый Интернет» (клиент INCY через CDN). Архитектурно изолирован от AmneziaWG. |
...
1. **Быстрый доступ:** Ключ `vpn://` встроен в карточку устройства (блок с моноширинным текстом — нажмите, чтобы скопировать).
2. **Резервный доступ:** Кнопка **«🔄 Другой способ подключения»** — отправляет:
