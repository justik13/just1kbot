...
> **ОБЛАСТЬ ДЕЙСТВИЯ:** В текущей реализации проекта поддерживается только **AmneziaWG 2.0 (`amneziawg2`)**, управляемый через серверный API `kyoresuas/amnezia-api`. Обычный WireGuard не используется.
