...
class DeployRollbackTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.active / "venv").mkdir(); (self.active / "venv/runtime").write_text("previous-runtime\n")
    ...
    def run_deploy(self, mode="success", rollback="success", migrate=0, extra=None):
        env = os.environ.copy(); env.update({
            ...
            "ADAPTER_MODE":mode, "PATH":str(self.bin)+":"+os.environ["PATH"], "ROLLBACK_MODE":rollback, "TEST_MIGRATION_COMMAND":str(self.migrate),
    ...
    def test_snapshot_failure_stops_before_delete_sync(self):
        fake=self.tmp/"fail-bin"; fake.mkdir(); rs=fake/"rsync"; rs.write_text('#!/bin/sh\necho "$*" >> "$EVENTS"\nexit 1\n'); rs.chmod(0o755)
    ...
    def test_stop_precedes_first_active_rsync(self):
        r=self.run_deploy(); events=self.events.read_text(); self.assertEqual(r.returncode,0); self.assertLess(events.index("stop"),events.index("rsync", events.index("stop")))
    def test_start_on_active_service_is_noop(self):
        env=os.environ.copy(); env.update({"ADAPTER_STATE":str(self.state),"ADAPTER_MODE":"success","EVENTS":str(self.events),"HEARTBEAT_FILE":str(self.active/".heartbeat")})
        subprocess.run([str(self.adapter),"start"],env=env,check=True); first=(self.state/"gen").read_text(); subprocess.run([str(self.adapter),"start"],env=env,check=True); self.assertEqual((self.state/"gen").read_text(),first)
    def test_new_pid_differs_from_old_pid(self):
        r=self.run_deploy(); self.assertEqual(r.returncode,0); self.assertEqual((self.state/"gen").read_text().strip(),"1"); self.assertIn("previous_mainpid=1000",r.stdout)
    def test_stop_failure_blocks_all_mutation(self):
        before_app=(self.active/"app.txt").read_bytes(); before_runtime=(self.active/"venv/runtime").read_bytes(); r=self.run_deploy("stop_fail")
        self.assertNotEqual(r.returncode,0); self.assertEqual((self.active/"app.txt").read_bytes(),before_app); self.assertEqual((self.active/"venv/runtime").read_bytes(),before_runtime); self.assertNotIn("migrate",self.events.read_text()); self.assertNotIn("deployment readiness=success",r.stdout)
    def test_rollback_previous_gets_new_pid(self):
        r=self.run_deploy("crash","success"); self.assertEqual(r.returncode,1); self.assertEqual((self.state/"gen").read_text().strip(),"2"); self.assertEqual((self.active/"app.txt").read_text(),"previous\n")
    ...
    def test_update_preserves_redis_credential(self):
        ...
        r=self.run_deploy_function('REDIS_PASSWORD=NEW_CANARY; setup_redis false', {"PATH":str(self.bin)+":"+os.environ["PATH"]})
        self.assertEqual(r.returncode,0,r.stdout+r.stderr); self.assertIn("requirepass OLD_CANARY",conf.read_text()); self.assertNotIn("NEW_CANARY",conf.read_text()+r.stdout+r.stderr)
    def test_arbitrary_update_password_cannot_desynchronize_env(self):
        conf=self.tmp/"redis.conf"; conf.write_text("requirepass EXISTING_VALUE\n")
        ctl=self.bin/"systemctl"; ctl.write_text("#!/bin/sh\nexit 0\n"); ctl.chmod(0o755)
        before=(self.active/".env").read_bytes(); r=self.run_deploy_function('ENV_FILE="'+str(self.active/'.env')+'"; REDIS_PASSWORD=UNRELATED_VALUE; determine_install_kind && setup_redis "$INITIAL_INSTALL"', {"PATH":str(self.bin)+":"+os.environ["PATH"]})
