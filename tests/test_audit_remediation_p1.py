...
class AuditRemediationP1Tests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_xray_api_idempotency_inflight_lock(self):
        """xray_api _get_inflight_op_lock returns distinct locks per key and serializes concurrent calls."""
        import sys
        from pathlib import Path
        sys.path.insert(0, str(Path(__file__).parent.parent / "scripts" / "xray_api"))
        try:
            ...
            if sys.path and "xray_api" in sys.path[0]:
                sys.path.pop(0)

    async def test_xray_api_idempotency_inflight_lock_eviction(self):
        ...
        try:
            ...
            try:
                ...
                self.assertIn("active-key", app._inflight_op_locks)
                self.assertIs(app._inflight_op_locks["active-key"], active_lock)
            finally:
                active_lock.release()
                app._inflight_op_locks.clear()
        finally:
