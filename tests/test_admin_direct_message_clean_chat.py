...
@pytest.mark.asyncio
async def test_admin_direct_message_uses_clean_chat_and_notice_in_hub():
    """
    ...
    with patch("bot.handlers.admin.users.message_routes.is_admin", return_value=True), \
        ...
        notice = kwargs.get("notice") or mock_show_card.call_args[0][3] if len(mock_show_card.call_args[0]) > 3 else kwargs.get("notice")
        assert notice is not None
        assert "902161217" in notice
        assert "успешно отправлено" in notice
