...
class TariffValueCalculatorTests(unittest.TestCase):
    def test_purchase(self):
        result = calculate_tariff_value(source_paid_hours=0, source_paid_value_rub=0,
    ...
    def test_same_tariff_renew(self):
        result = calculate_tariff_value(source_paid_hours=120, source_paid_value_rub=50,
    ...
    def test_upgrade_and_downgrade(self):
        upgrade = calculate_tariff_value(source_paid_hours=360, source_paid_value_rub=150,
            source_tariff=snap(), target_tariff=snap(2, 720, "600"), confirmed_additional_payment_rub=450, bonus_hours=7)
        downgrade = calculate_tariff_value(source_paid_hours=360, source_paid_value_rub=300,
    ...
    def test_due_rounds_up_and_hours_round_down(self):
        result = calculate_tariff_value(source_paid_hours=1, source_paid_value_rub=Decimal("0.01"),
            source_tariff=snap(hours=100, price="1"), target_tariff=snap(2, 3, "2"),
            confirmed_additional_payment_rub=Decimal("1.99"), bonus_hours=0)
    ...
    def test_rejections(self):
        base = dict(source_paid_hours=0, source_paid_value_rub=0, source_tariff=None,
                ...
                for hours in (1, 24, 719, 720, 8760):
                    for remaining in (0, 1, hours // 2, hours):
                        ...
                        result = calculate_tariff_value(
                            source_paid_hours=remaining, source_paid_value_rub=source_value,
                            source_tariff=source, target_tariff=snap(2, hours, target_price),
                            confirmed_additional_payment_rub=Decimal(target_price), bonus_hours=168)
                        self.assertTrue(result.invariant_holds)
                        self.assertLessEqual(result.paid_value_after_rub,
                                             source_value + Decimal(target_price))
                        self.assertLess(result.rounding_loss_hours, 1)
