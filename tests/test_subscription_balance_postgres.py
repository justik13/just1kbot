...
class SubscriptionBalancePostgresTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def seed(self, *, bonus=False, active=True, ledger=True, count=1):
        ...
        end=(grant_created_at+timedelta(days=30*count)) if not active else as_of+timedelta(days=30*count)
        async with self.sessions.begin() as s:
            tariff=(await s.execute(text("INSERT INTO tariffs(name,duration_days,device_limit,price_rub,is_active,sort_order,created_at) VALUES('T',30,2,90,true,0,:n) RETURNING id"),{"n":grant_created_at})).scalar_one()
            user=(await s.execute(text("INSERT INTO users(telegram_id,device_limit,referral_days,is_banned,is_bot_blocked,is_deleted,notification_retry_count,created_at,notified_3d,notified_1d,notified_2h,notified_expired,notified_grace_12h,device_creations_today,subscription_end) VALUES(:tg,2,0,false,false,false,0,:n,false,false,false,false,false,0,:e) RETURNING id"),{"tg":uuid.uuid4().int%10**12,"n":grant_created_at,"e":end})).scalar_one()
            for i in range(count):
                version=(await s.execute(text("INSERT INTO tariff_versions(tariff_id,version_number,name_snapshot,duration_hours,device_limit,price_rub,currency,created_at) VALUES(:t,:v,'T',720,2,:price,'RUB',:n) RETURNING id"),{"t":tariff,"v":i+1,"price":90+i,"n":grant_created_at})).scalar_one()
                quote=(await s.execute(text("INSERT INTO tariff_quotes(public_id,user_id,operation_type,target_tariff_version_id,current_paid_hours,current_paid_value_rub,bonus_hours,confirmed_payment_required_rub,resulting_paid_hours,resulting_paid_value_rub,resulting_bonus_hours,rounding_loss_hours,rounding_loss_value_rub,currency,status,expires_at,created_at,consumed_at) VALUES(:pub,:u,:op,:v,0,0,0,:price,720,:price,0,0,0,'RUB','consumed',:x,:n,:n) RETURNING id"),{"pub":uuid.uuid4(),"u":user,"op":"purchase" if i==0 else "renew","v":version,"price":90+i,"x":grant_created_at+timedelta(minutes=15),"n":grant_created_at})).scalar_one()
                payment=(await s.execute(text("INSERT INTO payments(user_id,tariff_id,tariff_quote_id,tariff_version_id,amount,currency,status,provider_status,fulfillment_status,reconciliation_status,checkout_status,snapshot_duration_days,snapshot_device_limit,snapshot_amount,snapshot_currency,referral_user_bonus_days,referral_referrer_bonus_days,created_at,updated_at) VALUES(:u,:t,:q,:v,:price,'RUB','completed','succeeded','succeeded','ok','active',30,2,:price,'RUB',0,0,:n,:n) RETURNING id"),{"u":user,"t":tariff,"q":quote,"v":version,"price":90+i,"n":grant_created_at})).scalar_one()
                await s.execute(text("UPDATE tariff_quotes SET payment_id=:p WHERE id=:q"),{"p":payment,"q":quote})
                if ledger:
                    await s.execute(text("INSERT INTO paid_value_ledger(user_id,source_type,source_id,entry_type,paid_hours_delta,paid_value_rub_delta,currency,tariff_version_id,quote_id,payment_id,metadata,created_at) VALUES(:u,'payment',:ps,'confirmed_payment',720,:price,'RUB',:v,:q,:p,'{}',:n)"),{"u":user,"ps":str(payment),"price":90+i,"v":version,"q":quote,"p":payment,"n":grant_created_at})
                await s.execute(text("INSERT INTO entitlement_entries(beneficiary_user_id,source_type,source_id,entry_type,days_delta,device_limit_snapshot,tariff_id_snapshot,metadata,created_at) VALUES(:u,'payment',:p,'payment_grant',30,2,:t,'{}',:n) ON CONFLICT DO NOTHING"),{"u":user,"p":str(payment),"t":tariff,"n":grant_created_at})
            if bonus:
                await s.execute(text("INSERT INTO entitlement_entries(beneficiary_user_id,source_type,source_id,entry_type,days_delta,metadata,created_at) VALUES(:u,'payment','bonus','referral_user_bonus',5,'{}',:n)"),{"u":user,"n":grant_created_at})
    ...
    async def real_grant(self, session, *, price=Decimal("90"), number=10,
                         user=None, tariff=None, do_grant=True):
        if user is None:
        ...
        version=TariffVersion(tariff_id=tariff.id,version_number=number,name_snapshot=tariff.name,duration_hours=720,device_limit=2,price_rub=price,currency="RUB"); session.add(version); await session.flush()
        now=now_utc(); quote=TariffQuote(public_id=uuid.uuid4(),user_id=user.id,operation_type="purchase" if user.subscription_end is None else "renew",target_tariff_version_id=version.id,current_paid_hours=0,current_paid_value_rub=0,bonus_hours=0,confirmed_payment_required_rub=price,resulting_paid_hours=720,resulting_paid_value_rub=price,resulting_bonus_hours=0,rounding_loss_hours=0,rounding_loss_value_rub=0,currency="RUB",status="active",created_at=now,expires_at=now+timedelta(minutes=15)); session.add(quote); await session.flush()
    ...
    async def test_application_fulfillment_rollback_is_atomic(self):
        async with self.sessions() as session:
            transaction=await session.begin(); user,_,_,_=await self.real_grant(session,number=12); uid=user.id; await transaction.rollback()
        async with self.sessions() as session:
    ...
    async def test_entitlement_invalid_sign_is_rejected(self):
        ...
        with self.assertRaises(Exception):
            async with self.sessions.begin() as s:
                await s.execute(text("INSERT INTO entitlement_entries(beneficiary_user_id,source_type,source_id,entry_type,days_delta,metadata) VALUES(:u,'manual','bad-sign','manual_grant',-1,'{}')"),{"u":u})
    ...
    async def test_raw_consistent_history_projects_one_lot(self):
        u,n,_=await self.seed()
        async with self.sessions() as s: self.assertEqual(len((await get_subscription_balance_snapshot(s,user_id=u,as_of=n)).paid_lots),1)
    ...
    async def test_expired_legacy_is_zero(self):
        ...
        async with self.sessions() as s:
            ...
            self.assertEqual((snapshot.tracked,snapshot.remaining_paid_hours,
                snapshot.remaining_paid_value_rub,snapshot.remaining_bonus_hours),(True,0,Decimal("0"),0))
    ...
    async def test_raw_projection_reads_immutable_tariff_version(self):
        u,n,_=await self.seed()
        async with self.sessions() as s: self.assertEqual((await get_subscription_balance_snapshot(s,user_id=u,as_of=n)).paid_lots[0].tariff_version_id,1)
    ...
    async def test_real_referral_after_grant_projects_zero_value_bonus(self):
        async with self.sessions.begin() as session:
            referrer=User(telegram_id=500000+uuid.uuid4().int%99999); tariff=Tariff(name="referral-real",duration_days=30,device_limit=2,price_rub=90,is_active=True)
            user=User(telegram_id=400000+uuid.uuid4().int%99999,referred_by=referrer.telegram_id)
            session.add_all((referrer,tariff,user)); await session.flush()
            user,_,payment,_=await self.real_grant(session,user=user,tariff=tariff,number=1)
            operation=PaymentFulfillmentOperation(payment_id=payment.id,operation_type="grant_referral",status="processing",idempotency_key=f"real-referral:{payment.id}",payload={},attempts=1,max_attempts=3,next_attempt_at=now_utc(),locked_by="test",locked_at=now_utc()); session.add(operation); await session.flush(); await referral(session,operation)
            start=await session.scalar(select(func.min(EntitlementEntry.created_at)).where(EntitlementEntry.beneficiary_user_id==user.id))
            snapshot=await get_subscription_balance_snapshot(session,user_id=user.id,as_of=start)
            self.assertEqual(snapshot.remaining_bonus_hours,120); self.assertTrue(all(lot.paid_value_rub==0 for lot in snapshot.bonus_lots))

    async def test_real_payment_a_b_reverse_b_leaves_only_a(self):
        async with self.sessions.begin() as session:
            user,tariff,_,_=await self.real_grant(session,number=1)
            _,_,payment_b,_=await self.real_grant(session,price=Decimal("300"),number=2,user=user,tariff=tariff)
            await self.real_reverse(session,payment_b)
            snapshot=await get_subscription_balance_snapshot(session,user_id=user.id,as_of=now_utc())
            self.assertTrue(snapshot.tracked); self.assertEqual(len(snapshot.paid_lots),1); self.assertEqual(snapshot.paid_lots[0].original_paid_value_rub,Decimal("90")); self.assertLessEqual(snapshot.remaining_paid_value_rub,Decimal("90"))
    ...
    async def test_real_payment_a_b_reverse_a_fails_cross_source(self):
        async with self.sessions.begin() as session:
            ...
            await self.real_grant(session,price=Decimal("300"),number=2,user=user,tariff=tariff)
            await self.real_reverse(session,payment_a)
            snapshot=await get_subscription_balance_snapshot(session,user_id=user.id,as_of=now_utc())
            self.assertEqual((snapshot.failure_code,snapshot.remaining_paid_value_rub),("reversal_crosses_unrelated_segments",None))

    async def test_for_update_serializes_with_real_grant(self):
        async with self.sessions.begin() as session:
            user,tariff,_,_=await self.real_grant(session,number=1)
            _,_,payment_b,operation=await self.real_grant(session,price=Decimal("300"),number=2,user=user,tariff=tariff,do_grant=False)
            user_id=user.id; operation_id=operation.id
        locked=asyncio.Event(); release=asyncio.Event()
        async def reader():
            async with self.sessions.begin() as session:
                await get_subscription_balance_snapshot(session,user_id=user_id,as_of=now_utc(),for_update=True); locked.set(); await release.wait()
        async def fulfill():
            async with self.sessions.begin() as session:
                operation=await session.get(PaymentFulfillmentOperation,operation_id)
                with patch("services.payment_fulfillment.queue_post_commit_task"): await grant(session,operation)
        reader_task=asyncio.create_task(reader()); await locked.wait(); grant_task=asyncio.create_task(fulfill()); await asyncio.sleep(.1); self.assertFalse(grant_task.done()); release.set(); await reader_task; await grant_task
        async with self.sessions() as session:
            self.assertEqual(await session.scalar(select(func.count(EntitlementEntry.id)).where(EntitlementEntry.source_id==str(payment_b.id),EntitlementEntry.entry_type=="payment_grant")),1)
            self.assertEqual(await session.scalar(select(func.count(PaidValueLedgerEntry.id)).where(PaidValueLedgerEntry.payment_id==payment_b.id,PaidValueLedgerEntry.entry_type=="confirmed_payment")),1)
