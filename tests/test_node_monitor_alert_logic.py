...
class NodeMonitorAlertLogicTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_transient_glitch_no_alert(self):
    ...
    async def test_confirmed_failure_sends_warning_alert(self):
        ...
        with patch("services.workers.node_monitor.get_all_servers", return_value=[server]), \
            ...
            self.assertIn("admin_server_card:3", inline_buttons)
            self.assertIn("admin_dismiss_alert", inline_buttons)

            state = get_server_monitor_state(3)
            self.assertEqual(state.health_state, ServerHealthState.PROBLEM)

    async def test_problem_observation_window_silence_and_flapping_protection(self):
    ...
    async def test_problem_timeout_escalates_to_auto_disabled(self):
        ...
        with patch("services.workers.node_monitor.get_all_servers", return_value=[server]), \
            ...
            self.assertFalse(mock_update.call_args.kwargs["is_active"])
            self.assertEqual(mock_update.call_args.kwargs["disabled_reason"], "AUTO_UNAVAILABLE")
    ...
    async def test_dismiss_admin_alert_handler(self):
