...
class TestPr160Regressions(unittest.IsolatedAsyncioTestCase):
    async def test_legacy_profile_callbacks_redirect_to_hub(self):
        ...
        with patch("bot.handlers.start.back_to_main_menu", new=AsyncMock()) as hub:
            await fallback.legacy_profile_callback(callback, state, session, db_user)

        hub.assert_awaited_once_with(callback, state, db_user, session)
    ...
    async def test_back_to_main_menu_rejects_missing_session_before_db_render(self):
        ...
        await start.back_to_main_menu(callback, state, db_user=db_user, session=None)
    ...
    async def test_white_internet_callback_is_alert(self):
        ...
        callback.answer.assert_awaited_once_with("🔨 Раздел находится в разработке", show_alert=True)

    def test_hub_keyboard_admin_and_non_admin_access(self):
        for is_active in (False, True):
            for mtproto_url in (None, "https://example.test/mtproto"):
                admin_keyboard = get_hub_keyboard(is_admin=True, is_active=is_active, mtproto_url=mtproto_url)
                admin_buttons = [button for row in admin_keyboard.inline_keyboard for button in row]
                admin_callbacks = [button.callback_data for button in admin_buttons if button.callback_data is not None]
                self.assertIn("menu_admin", admin_callbacks)
                self.assertEqual(admin_callbacks[-2:], ["white_internet", "menu_admin"])
                self.assertEqual(len(admin_callbacks), len(set(admin_callbacks)))

                user_keyboard = get_hub_keyboard(is_admin=False, is_active=is_active, mtproto_url=mtproto_url)
                user_buttons = [button for row in user_keyboard.inline_keyboard for button in row]
                user_callbacks = [button.callback_data for button in user_buttons if button.callback_data is not None]
    ...
    async def test_admin_menu_callback_rejects_non_admin(self):
        ...
        with patch.object(dashboard, "is_admin", return_value=False), patch.object(dashboard, "_show_admin_dashboard", new=AsyncMock()) as show_dashboard:
            await dashboard.show_admin_menu(callback, state, session)

        callback.answer.assert_awaited_once_with(texts.ERROR_ACCESS_DENIED, show_alert=True)
    ...
    async def test_balance_purchase_back_cancels_commits_and_then_navigates(self):
        ...
        with (
            patch.object(purchase_routes, "cancel_account_purchase_quote", new=AsyncMock(return_value=quote)) as cancel_quote,
            patch.object(purchase_routes, "show_tariff_showcase_callback", new=AsyncMock()) as show_showcase,
        ):
            await purchase_routes.cancel_purchase(callback, session, user)

        cancel_quote.assert_awaited_once_with(session, user_id=42, quote_public_id=quote_id)
    ...
    async def test_balance_purchase_back_does_not_navigate_when_cancel_fails(self):
        ...
        with (
            patch.object(purchase_routes, "cancel_account_purchase_quote", new=AsyncMock(side_effect=AccountPurchaseError("quote_not_active"))),
            patch.object(purchase_routes, "show_tariff_showcase_callback", new=AsyncMock()) as show_showcase,
        ):
            await purchase_routes.cancel_purchase(callback, session, user)
    ...
    def test_device_download_file_action_is_preserved(self):
        ...
        ready_callbacks = [button.callback_data for row in ready.inline_keyboard for button in row]
    ...
    def test_legal_urls_and_faq_match_current_navigation(self):
        ...
        for key in texts.get_all_text_keys():
            ...
            self.assertTrue(set(found_urls).issubset(allowed_urls), msg=f"Unexpected legal URL in {key}: {found_urls}")

    async def test_hub_renders_admin_button_only_for_configured_admin(self):
        admin = SimpleNamespace(id=10, telegram_id=100, first_name="Admin", referred_by=None, subscription_end=None, device_limit=2)
        user = SimpleNamespace(id=11, telegram_id=101, first_name="User", referred_by=None, subscription_end=None, device_limit=2)
        ...
        admin_callbacks = [button.callback_data for row in admin_kb.inline_keyboard for button in row if button.callback_data]
        user_callbacks = [button.callback_data for row in user_kb.inline_keyboard for button in row if button.callback_data]
        self.assertIn("menu_admin", admin_callbacks)
        self.assertNotIn("menu_admin", user_callbacks)

    async def test_hub_renders_referrer(self):
        user = SimpleNamespace(id=10, telegram_id=100, first_name="User", referred_by=200, subscription_end=None, device_limit=2)
        referrer = SimpleNamespace(telegram_id=200, first_name="Referrer", username="ref")
        balance = SimpleNamespace(real_available=123, bonus_available=7)
        ...
        with (
            patch.object(start.SubscriptionService, "check_access", new=AsyncMock(return_value=True)),
            patch.object(start, "get_account_balance", new=AsyncMock(return_value=balance)),
            patch("database.repositories.profiles_repo.get_user_profiles", new=AsyncMock(return_value=[object()])),
