...
class ExecutorPostgresTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.old_sessionmaker=connection._sessionmaker; connection._sessionmaker=self.sessions
    ...
    async def queued_create(self, *, active=True):
        async with self.sessions.begin() as s:
            ...
            s.add(p); await s.flush()
            op=APIOperation(operation_type="create_peer",status="pending",idempotency_key=f"e2e-{p.id}",server_id=self.server_id,profile_id=p.id,server_name_snapshot="fake",api_url_snapshot="https://fake",api_key_snapshot="key",client_name="exact",payload={"desired_version":1})
            s.add(op); await s.flush(); return p.id,op.id
    async def claim_one(self):
        return (await claim_api_operations(worker_id="e2e",limit=1,session_factory=self.sessions))[0]
    async def ready_retry(self, oid):
        async with self.sessions.begin() as s:
            op=await s.get(APIOperation,oid); op.next_attempt_at=datetime.now(timezone.utc)
    async def test_ambiguous_create_reconciles_without_duplicate(self):
        pid,oid=await self.queued_create(); config=self.valid_config()
        class Fake:
            def __init__(self): self.peers={}; self.posts=self.deletes=0
            async def get_all_clients(self): return [SimpleNamespace(id=i,clientName=n) for i,n in self.peers.items()]
            async def create_user_result(inner,name,expires):
                inner.posts+=1; peer=f"p{inner.posts}"; inner.peers[peer]=name
                if inner.posts==1: return AmneziaAPIResult(False,None,AmneziaErrorKind.TIMEOUT,None,True,True)
                return AmneziaAPIResult(True,AmneziaClientCreateResponse(id=peer,config=config),None,201,False,False)
    ...
    async def test_invalid_config_cleanup_retry(self):
        ...
        class Fake:
            def __init__(self): self.peers={"bad":"exact"}; self.posts=0; self.deletes=0
            async def get_all_clients(self): return [SimpleNamespace(id=i,clientName=n) for i,n in self.peers.items()]
            async def create_user_result(inner,name,expires): inner.posts+=1; return AmneziaAPIResult(True,AmneziaClientCreateResponse(id="bad",config="invalid"),None,201,False,False)
            async def delete_user_result(inner,peer):
                inner.deletes+=1
                if inner.deletes==1: return AmneziaAPIResult(False,None,AmneziaErrorKind.NETWORK_ERROR,None,True,False)
                inner.peers.pop(peer,None); return AmneziaAPIResult(True,None,None,204,False,False)
        fake=Fake()
        with patch("services.api_operations_executor._client",return_value=fake):
            await execute_claimed_api_operation(await self.claim_one()); await self.ready_retry(oid)
            await execute_claimed_api_operation(await self.claim_one())
        async with self.sessions() as db:
            self.assertEqual((await db.get(VPNProfile,pid)).provisioning_status,"create_failed")
        self.assertEqual(fake.posts,1); self.assertFalse(fake.peers)
    async def test_profile_missing_after_post_is_compensated(self):
        pid,oid=await self.queued_create(); config=self.valid_config(); sessions=self.sessions
        class Fake:
            def __init__(self): self.peers={}; self.deletes=0
            async def create_user_result(inner,name,expires):
                inner.peers["made"]=name
                async with sessions.begin() as db:
                    profile=await db.get(VPNProfile,pid); await db.delete(profile)
                return AmneziaAPIResult(True,AmneziaClientCreateResponse(id="made",config=config),None,201,False,False)
            async def delete_user_result(inner,peer): inner.deletes+=1; inner.peers.pop(peer,None); return AmneziaAPIResult(True,None,None,204,False,False)
        ...
        self.assertFalse(fake.peers); self.assertEqual(fake.deletes,1)
    async def test_inactive_pending_create_does_not_post(self):
        pid,oid=await self.queued_create(active=False)
        fake=SimpleNamespace(create_user_result=unittest.mock.AsyncMock(),get_all_clients=unittest.mock.AsyncMock(return_value=[]))
        with patch("services.api_operations_executor._client",return_value=fake): await execute_claimed_api_operation(await self.claim_one())
        fake.create_user_result.assert_not_awaited()
        async with self.sessions() as db: self.assertIsNone(await db.get(VPNProfile,pid))

    async def test_terminal_create_failure_updates_profile(self):
        pid,oid=await self.create_claimed("create_peer")
        await finalize_operation_failure(oid,worker_id="worker",expected_attempt_number=1,retryable=False,error_code="validation_failed",error_message="safe",session_factory=self.sessions)
        async with self.sessions() as db: self.assertEqual((await db.get(VPNProfile,pid)).provisioning_status,"create_failed")
    async def test_terminal_update_failure_updates_profile(self):
        ...
        await finalize_operation_failure(oid,worker_id="worker",expected_attempt_number=1,retryable=False,error_code="auth_failed",error_message="safe",session_factory=self.sessions)
        async with self.sessions() as db: self.assertEqual((await db.get(VPNProfile,pid)).provisioning_status,"update_failed")
