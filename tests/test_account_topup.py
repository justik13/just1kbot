...
class AccountTopupProviderTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_auto_fulfillment_white_internet_pack_success(self):
        from unittest.mock import MagicMock, patch
        ...
        p.provider_confirmed_at = datetime(2026, 8, 2, 6, tzinfo=timezone.utc)
        p.credit_notified_at = None
        p.topup_context = {"auto_fulfill_action": "white_internet_pack", "pack_gb": 10}
        ...
        with (
            ...
            patch("services.referral_bonus.grant_referral_bonus_for_topup", new=AsyncMock(return_value=0)),
            patch("services.audit_service.AuditService.log_action", new=AsyncMock()),
            patch("services.white_internet_service.WhiteInternetService.topup_quota", new=AsyncMock(return_value=(True, "Success", MagicMock()))) as mock_pack,
            patch("database.connection.queue_post_commit_task", new=MagicMock()),
        ):
            created, balance = await settle_succeeded_topup(
                session,
            ...
            self.assertTrue(created)
            self.assertEqual(p.topup_context.get("auto_fulfill_status"), "succeeded")
            mock_pack.assert_awaited_once_with(session, user_id=p.user_id, pack_gb=10, actor_telegram_id=777)
            session.begin_nested.assert_not_called()
