...
class AmneziaBridgeDisabledTests(unittest.IsolatedAsyncioTestCase):
    async def test_bridge_endpoint_returns_404_without_hmac_secret(self):
        env = {
            "BOT_TOKEN": "123:test",
            "REDIS_URL": "redis://localhost:6379/1",
            "REDIS_PASSWORD": "test",
        ...
        with patch.dict(os.environ, env, clear=True):
            ...
            get_settings.cache_clear()
            try:
                response = await amnezia_bridge_handler(object())
            finally:
                get_settings.cache_clear()
