...
class BackupOperationsContractTests(unittest.TestCase):
    ...
    def test_dump_list_precedes_publication(self):
        self.assertLess(
            self.backup.index("pg_restore --list"),
            self.backup.index('mv -- "$local_partial" "$final"'),
        )
    ...
    def test_encryption_failure_cannot_publish(self):
        self.assertLess(
            self.backup.rindex('age -r "$BACKUP_AGE_RECIPIENT"'),
            self.backup.index('mv -- "$local_partial" "$final"'),
        )

    def test_retention_follows_all_required_publication(self):
        self.assertLess(
            self.backup.index("required off-site publication failed"),
            self.backup.index("mapfile -t expired"),
        )
    ...
    def test_rehearsal_database_removed_after_success(self):
        self.assertIn(
            'dropdb --force --if-exists --maintenance-db="$MAINTENANCE_DATABASE" "$test_db"',
            self.rehearsal,
        )
    ...
    def test_deploy_uses_systemd_timer_not_backup_cron(self):
        ...
        self.assertNotIn(
            'echo "0 3 * * * /usr/local/bin/just1kbot-backup.sh"', self.deploy
        )
...
class BackupFailurePathTests(unittest.TestCase):
    def setUp(self):
        ...
        self.envfile.write_text(
            "DATABASE_URL='postgresql://user:pass@localhost/source'\nDB_ENCRYPTION_KEY='canary'\nREDIS_URL='redis://test'\nBOT_TOKEN='secret-canary'\n"
        )
        self.log = self.root / "calls"
        self.env = os.environ | {
            "PATH": f"{self.bin}:{os.environ['PATH']}",
            "ENV_FILE": str(self.envfile),
            "BACKUP_DIR": str(self.backups),
            "BACKUP_LOCK_FILE": str(self.root / "lock"),
            "BACKUP_AGE_RECIPIENT": "age1testrecipient",
            "CALL_LOG": str(self.log),
            "REVISION_FILE": str(self.root / "revisions"),
        }
        (self.root / "revisions").write_text("rev_one\nrev_one\n")
        self._shim(
            "age",
            """out=""; input=""; while (($#)); do case "$1" in -o) out=$2; shift 2;; -r|-i) shift 2;; -d) shift;; *) input=$1; shift;; esac; done; [[ ${AGE_FAIL:-0} != 1 ]] || { : >"$out"; exit 9; }; if [[ -n "$input" ]]; then cp "$input" "$out"; else printf encrypted >"$out"; fi""",
        )
        self._shim(
            "pg_dump",
            """echo pg_dump >>"$CALL_LOG"; if [[ ${1:-} == --version ]]; then echo 'pg_dump (PostgreSQL) 16.4'; exit; fi; for a in "$@"; do [[ $a == --file=* ]] && printf dump >"${a#--file=}"; done""",
        )
        self._shim(
            "pg_restore",
            """echo pg_restore >>"$CALL_LOG"; exit ${PG_RESTORE_FAIL:-0}""",
        )
        self._shim(
            "psql",
            '''echo psql >>"$CALL_LOG"; n=$(cat "${REVISION_INDEX:-/dev/null}" 2>/dev/null || echo 1); mapfile -t r <"$REVISION_FILE"; echo "${r[$((n-1))]:-rev_one}"; echo $((n+1)) >"${REVISION_INDEX:-/dev/null}"''',
        )
    ...
    def visible(self, directory=None):
        return list((directory or self.backups).glob("*.tar.age"))
    ...
    def test_locked_second_process_never_calls_dump(self):
        ...
        try:
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertNotIn(
                "pg_dump", self.log.read_text() if self.log.exists() else ""
            )
        finally:
            holder.terminate()
            holder.wait()
    ...
    def test_required_offsite_failure_removes_local_pair(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(self.visible(), [])
        self.assertNotIn("tail", self.log.read_text())
        ...
        for stage in ("chmod", "sidecar_rename", "artifact_rename"):
            for required in ("true", "false"):
                with self.subTest(stage=stage, required=required):
                    offsite = self.root / f"offsite-{stage}-{required}"
                    self.log.unlink(missing_ok=True)
                    self._shim(
                        "chmod",
                        '''if [[ ${FAIL_STAGE:-} == chmod && "$*" == *"$BACKUP_OFFSITE_DIR"* ]]; then exit 9; fi; exec /bin/chmod "$@"''',
                    )
                    self._shim(
                        "mv",
                        '''src=${2:-}; if [[ ${FAIL_STAGE:-} == sidecar_rename && $src == "$BACKUP_OFFSITE_DIR"/*.sha256.partial ]]; then exit 9; fi; if [[ ${FAIL_STAGE:-} == artifact_rename && $src == "$BACKUP_OFFSITE_DIR"/*.tar.age.partial ]]; then exit 9; fi; exec /bin/mv "$@"''',
                    )
                    result = self.run_backup(
                        BACKUP_OFFSITE_DIR=offsite,
                        BACKUP_REQUIRE_OFFSITE=required,
                        FAIL_STAGE=stage,
                    )
                    self.assertEqual(
                        result.returncode == 0, required == "false", result.stderr
                    )
                    self.assertEqual(
                        len(self.visible()), 1 if required == "false" else 0
                    )
                    self.assertEqual(
                        len(list(self.backups.glob("*.sha256"))),
                        1 if required == "false" else 0,
                    )
                    self.assertEqual(self.visible(offsite), [])
                    self.assertEqual(list(offsite.glob("*.sha256")), [])
    ...
    def _archive(self, member_kind="safe"):
        ...
        artifact = self.root / "bad.tar.age"
        shutil.copy(bundle, artifact)
        artifact.chmod(0o600)
        (self.root / "bad.tar.age.sha256").write_text(
            f"{__import__('hashlib').sha256(artifact.read_bytes()).hexdigest()}  bad.tar.age\n"
        )
        identity = self.root / "identity"
        identity.write_text("id")
        return artifact, os.environ | {
            "PATH": f"{self.bin}:{os.environ['PATH']}",
    ...
    def _valid_archive(self, internal_lines=None, external_text=None):
        ...
        (payload / "manifest.json").write_text(
            json.dumps(
                {
                    "format_version": 1,
                    "created_at_utc": "20260729T000000Z",
                    "database_name": "db",
                    "postgresql_version": "16",
                    "alembic_revision": "rev_one",
                    "git_commit_sha": "abc",
                    "files": ["dump.custom", "config.env"],
                }
            )
        )
        correct = [
            f"{hashlib.sha256((payload / name).read_bytes()).hexdigest()}  {name}"
            for name in ("dump.custom", "config.env")
        ]
        ...
        artifact = self.root / "valid.tar.age"
        shutil.copy(bundle, artifact)
        artifact.chmod(0o600)
        digest = hashlib.sha256(artifact.read_bytes()).hexdigest()
        (self.root / "valid.tar.age.sha256").write_text(
            external_text if external_text is not None else f"{digest}  valid.tar.age\n"
        )
        identity = self.root / "valid-identity"
        identity.write_text("id")
        env = os.environ | {
            "PATH": f"{self.bin}:{os.environ['PATH']}",
            "AGE_IDENTITY_FILE": str(identity),
    ...
    def _rehearsal(self, drop_fail=False, revision="rev_one"):
        ...
        self._shim(
            "dropdb", '''[[ ${DROP_FAIL:-0} != 1 ]] || exit 9; rm -f "$DB_STATE"'''
        )
        self._shim("pg_restore", "exit 0")
        self._shim(
            "psql",
            f"""input=$(cat); args="$* $input"; if [[ $args == *pg_database* ]]; then [[ -e "$DB_STATE" ]] && echo 1 || echo 0; elif [[ $args == *information_schema* ]]; then echo 1; elif [[ $args == *"count(*) FROM alembic_version"* ]]; then echo 1; elif [[ $args == *"version_num FROM alembic_version"* ]]; then echo {revision}; else echo 1; fi""",
        )
        env = os.environ | {
            "PATH": f"{self.bin}:{os.environ['PATH']}",
            "VERIFY_BACKUP": str(verifier),
            "DB_STATE": str(self.root / "database"),
            "DROP_FAIL": "1" if drop_fail else "0",
            "REHEARSAL_CRITICAL_TABLES": "payments",
        }
        return subprocess.run(
            [OPS / "restore_rehearsal.sh", artifact],
            env=env,
            text=True,
            capture_output=True,
        ), self.root / "database"
    ...
    def test_legacy_cron_filter_preserves_other_lines(self):
        ...
        result = subprocess.check_output(
            ["awk", '!(NF == 6 && $6 == "/usr/local/bin/just1kbot-backup.sh")'],
            input=source,
            text=True,
        )
        self.assertNotIn("just1kbot-backup.sh", result)
        self.assertIn("just1kbot-healthcheck.sh", result)
        self.assertIn("/other --flag", result)
        self.assertIn("MAILTO", result)


@unittest.skipUnless(
    os.getenv("RUN_BACKUP_INTEGRATION") == "1", "requires PostgreSQL and age"
)
    ...
    def test_real_dump_verify_and_isolated_restore(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            recipient = subprocess.check_output(
                ["age-keygen", "-y", identity], text=True
            ).strip()
            envfile.write_text(
            ...
            subprocess.run(
                [
                    ...
                    url,
                    "-c",
                    f"CREATE TABLE IF NOT EXISTS backup_rehearsal_data(value text); TRUNCATE backup_rehearsal_data; INSERT INTO backup_rehearsal_data VALUES ('{preserved}');",
                ],
                check=True,
                capture_output=True,
            )
            subprocess.run(
                [
                    ...
                    url,
                    "-c",
                    "CREATE TABLE IF NOT EXISTS alembic_version(version_num varchar(32)); INSERT INTO alembic_version SELECT 'test' WHERE NOT EXISTS (SELECT 1 FROM alembic_version);",
                ],
                check=True,
                capture_output=True,
            )
            ...
            try:
                rehearsal = subprocess.run(
                    [OPS / "restore_rehearsal.sh", "--keep-test-db", artifact],
                    env=env,
                    text=True,
                    capture_output=True,
                    check=True,
                )
                self.assertIn("result=success", rehearsal.stdout)
                self.assertIn("cleanup=kept", rehearsal.stdout)
                ...
                restored = subprocess.check_output(
                    [
                        ...
                        kept_db,
                        "-Atc",
                        "SELECT value FROM backup_rehearsal_data",
                    ],
                    env=env,
                    text=True,
                ).strip()
                ...
                source_revision = subprocess.check_output(
                    ["psql", url, "-Atc", "SELECT version_num FROM alembic_version"],
                    text=True,
                ).strip()
                ...
                if kept_db:
                    subprocess.run(
                        ["dropdb", "--force", "--if-exists", kept_db],
                        env=env,
                        check=True,
                    )
            self.assertEqual(
                subprocess.check_output(
                    ["psql", url, "-Atc", "SELECT value FROM backup_rehearsal_data"],
                    text=True,
                ).strip(),
                preserved,
            )
            ...
            self.assertEqual(
                subprocess.check_output(
                    [
                        ...
                        "postgres",
                        "-v",
                        f"target_db={cleanup_probe}",
                    ],
                    env=env,
                    input="SELECT count(*) FROM pg_database WHERE datname = :'target_db';\n",
                    text=True,
                ).strip(),
                "1",
            )
            ...
            self.assertEqual(
                subprocess.check_output(
                    [
                        ...
                        url,
                        "-Atc",
                        f"SELECT count(*) FROM pg_database WHERE datname='{dbname}'",
                    ],
                    text=True,
                ).strip(),
                "0",
